...
impl OakleyGroup14 {
    ...
    ///
    /// RFC 3526 Oakley Group 14 is a Sophie Germain / safe-prime
    /// group: `p = 2q + 1`, so `q = (p - 1) / 2`. RFC 4556 §3.2.3.1
    /// requires the client to include `q` alongside `p` and `g`
    /// inside `DomainParameters` when presenting a DH
    /// SubjectPublicKeyInfo to the KDC — Windows AD KDCs reject the
    /// SPKI without it.
    pub fn order() -> BigUint {
        let p = Self::prime();
        let one = BigUint::from_u32(1);
        let two = BigUint::from_u32(2);
        let (q, _) = p.sub(&one).div_rem(&two);
        q
    }
