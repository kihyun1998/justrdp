...
#### S3d 이후 (예정)
- [-] WebSocket 전송 (브라우저 환경) — 현실 mstsc 트래픽 (16 bpp RLE) 까지 완전 처리. 8 bpp Palette 만 잔여 — S3d
- [ ] 8 bpp Palette PDU 지원 (`SessionEvent::Palette` 분리 + indexed → RGBA 변환) — S3d
- [ ] Surface Commands / Orders / RemoteFX (`justrdp_graphics::PlanarDecompressor`, `rfx`, `nscodec`, `clearcodec`, `avc`) — S3e
