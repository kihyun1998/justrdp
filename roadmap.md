...
0. [Product Milestones](#0-product-milestones)
...
## 0. Product Milestones

> **Reading guide:** §1–§22 below are the **technical inventory** organized
> by crate / protocol layer. Most sections apply to both Client and Server
> products. This §0 maps that inventory to two **product milestones** so
> work prioritisation has a clear product story.
>
> Each subsection (`§x.y.z`) carries a tag — `[M-Client]`, `[M-Server]`,
> or `[both]` — to indicate which milestone consumes it.

### Two milestones, in order

| | **M-Client** (primary) | **M-Server** (secondary) |
|---|---|---|
| Goal | RDP 접속이 가능한 production-quality 클라이언트 | 자체 RDP 서버 — native 앱을 RDP 클라이언트(mstsc 등)에 노출 |
| Deliverable | Tauri 데스크톱 클라이언트 (`justrdp-tauri`) + 브라우저 클라이언트 (`justrdp-web`) + native winit/wgpu (`justrdp-client`) — Windows RDS / 일반 RDP 서버 호환 | `justrdp-server` 데몬 — 가상 머신/원격 데스크톱 호스트, 멀티세션 lifecycle |
| Trigger | Phase 1~7 의 client-side 항목 + §11.5a Tauri MVP 출시 | M-Client production-quality 도달 후 |
| Status (2026-04-29) | 진행 중 — §4~§5 거의 완료, §6~§9 부분 완료, §11.3/§11.5a 시작 | 골격만 (`justrdp-server` 일부, §11.4) |

### M-Client 핵심 path (이 milestone 안에서 *반드시* 끝나야 함)

- **§4 Foundation** — PDU encode/decode, crypto. ✅ 거의 완료
- **§5 Connection & Authentication**
  - §5.1 connector state machine ✅
  - §5.2.1~5.2.7 인증 (CredSSP/NTLM/Kerberos/Standard RDP/RCG/RestrictedAdmin/AAD/RDSTLS) ✅
  - §5.3 실서버 시퀀스 검증 ✅ (P0 finalization wedge fix 2026-04-29 포함)
  - §5.4 TLS upgrade ✅
  - §5.5 sync I/O runtime (`justrdp-blocking`) ✅
  - **§5.6 async I/O runtime (`justrdp-async` + `justrdp-tokio`)** 🚧 — **8-9주 작업, M-Client 핵심 path** (web/Tauri/native 3개 클라이언트 트랙이 모두 의존)
- **§6 Standalone Codecs** — RLE / Planar / RFX / NSCodec / ClearCodec 디코딩 (server→client). H.264/AVC와 audio 코덱은 stretch
- **§7 Session Core** — `ActiveStage`, fast/slow-path, SVC/DVC ✅ 대부분
- **§8 Channels (client-side만)** — clipboard / rdpsnd 수신 / rdpdr / displaycontrol / EGFX / native backends. RAIL은 stretch
- **§9 Advanced** — multi-monitor / auto-reconnect / session redirection ✅ 대부분
- **§10.1 RD Gateway client (MS-TSGU)** — 기업 환경 접속 필수. §10.2 RDPEUDP/MT는 stretch
- **§11 Applications** — §11.3 web (브라우저), §11.5a Tauri (데스크톱 MVP), §11.5b winit/wgpu (데스크톱 secondary)
- **§13 Public API**, **§14 Testing**, **§17 Security audit** — production gate

### M-Client 에서 *제외* (M-Server 또는 deferred)

- §9.4 Touch / §9.5 Pen / §9.7 USB redirection — stretch, MVP에서 제외
- §9.6 Smartcard (PKINIT) — stretch (기업 환경 도입 시 활성)
- §6.10 Audio codecs (lossy AAC/Opus 인코딩) — 클라이언트는 디코딩만 필요
- §11.1 acceptor / §11.2 `justrdp-server` 내부 작업 — M-Server로
- License issuance / multi-session lifecycle / shadow — M-Server로
- Server-side encoders (RFX/EGFX 인코딩) — M-Server로

### M-Server 시작 trigger 와 in-scope

- **시작 조건**: M-Client `§11.5a Tauri` 가 production reach (Windows RDS/Azure VD/온프렘 RD 서버 모두 안정 접속) + 외부 사용자 issue 가 의미 있는 양으로 들어오고 stable.
- **In-scope**:
  - §11.1 `justrdp-acceptor` — accept loop, server-side connector
  - §11.2 `justrdp-server` extensible skeleton — Demand Active emit, RDP server runtime
  - §8.x 채널의 server-side 처리 (cliprdr/rdpsnd/rdpdr 송신 방향)
  - GFX/RFX 서버측 인코딩 (§6 코덱의 인코더 짝)
  - License Server (MS-RDPELE 풀 교환) — Appendix G.1 deferred → M-Server에서 활성
  - 멀티세션 lifecycle / shadow
  - §11.6 RD Gateway 서버 / §11.7 RDP proxy

### 이 §0 의 사용법

- 새 작업을 잡을 때 — **태그 `[M-Client]` 항목을 먼저** 선택. M-Server 태그는 M-Client 안정 후.
- 작업 priority 결정이 모호할 때 — "이게 M-Client core path인가, stretch인가, 아예 M-Server인가?" 로 묻기.
- §x.y.z 본문에 태그가 없으면 **둘 다(both)** 로 간주 (대부분 §4~§7 같은 sans-IO core).
- 새 phase/항목 추가 시 헤더에 태그 1개 명시 — 태그 없는 신규 항목 금지.
...
```
    ...
    └────────────────────────────────────────────────────────────────────┘
              │
    ┌─────────▼──────────────────────────────────────────────────────────┐
    │              I/O Adapters  (target end-state — §5.6)              │
    │                                                                    │
    │  justrdp-async  (planned)  — single pump WebClient<T>,             │
    │     trait WebTransport / TlsUpgrade<T> / CredsspDriver<T>          │
    │       │                                                            │
    │       ├── justrdp-tokio   (NativeTcp/Tls/Credssp + AsyncRdpClient) │
    │       ├── justrdp-web     (WebSocket + browser TLS, wasm-only)     │
    │       └── justrdp-blocking (legacy sync — kept for compat)         │
    │                                                                    │
    │  justrdp-tls     (sync TLS upgrade — used by justrdp-blocking)     │
    │                                                                    │
    │  Current state (2026-04-29): justrdp-blocking ✅ ships;            │
    │  justrdp-tokio v1 ✅ (spawn_blocking wrap) + native_*/Demo 호스팅;  │
    │  justrdp-async ✅ Phase 1 완료 (WebTransport family promoted);     │
    │  justrdp-web → wasm/렌더 전용으로 슬림화. Phase 2-6 진행 중.        │
    └────────────────────────────────────────────────────────────────────┘
              │
    ┌─────────▼──────────────────────────────────────────────────────────┐
...
| `justrdp-async`          | (§5.6.1 Phase 1 **완료 2026-04-29**) `WebTransport` family + `WebClient<T>` 펌프 + `ActiveSession<T>` 가 `justrdp-web` 에서 promote 됨. `no_std + alloc`. tokio/wasm/sync 모든 backend 의 공통 layer. `AsyncRead/Write/TlsUpgrader/CredsspDriver` 형태는 2026-04-29 거부됨 — §5.6.1 참조 |
| `justrdp-tokio`          | (§5.6.2 v1 **완료 2026-04-29**, v2 §5.6.4 planned) v1: `AsyncRdpClient` `spawn_blocking` wrap + `NativeTcpTransport`/`NativeTlsUpgrade`/`NativeCredsspDriver` 호스팅 (§5.6.1 Phase 1 에서 web 으로부터 이전). v2: `WebClient<NativeTcpTransport>` 직결 — Send+Sync, 게이트웨이/재연결/리다이렉트 포함, fan-out 친화. Tauri/Iced/eframe 임베더용 |
...
| `justrdp-client`  | 완전한 RDP 클라이언트 바이너리 (§11.5b winit/wgpu 트랙)    |
| `justrdp-tauri`   | (§11.5a, 미구현) Tauri webview 기반 데스크톱 클라이언트 바이너리. MVP 출시 트랙 |
| `justrdp-server`  | 확장 가능한 RDP 서버 스켈레톤                              |
| `justrdp-web`     | wasm/렌더 전용 (브라우저 RDP 클라이언트). §5.6.1 Phase 1 (2026-04-29) 이후 `WebTransport`/`WebClient`/`ActiveSession` 은 `justrdp-async` 로 이전, native 데스크톱 transport 는 `justrdp-tokio` 로 이전. 잔존: `audio.rs`, `clipboard.rs`, `render.rs`, `input.rs`, `canvas.rs`, `js.rs`, `websocket.rs` |
...
## 4. Phase 1 -- Foundation (Core Protocol)  `[both]`
...
## 5. Phase 2 -- Connection & Authentication  `[M-Client primary, M-Server reuse §5.1 via §11.1]`
...
### 5.6 `justrdp-async` / `justrdp-tokio` -- Async I/O Runtime  `[M-Client core path]`

> **requires**: 5.5 `justrdp-blocking` 안정화 ✅ (P0 finalization wedge fix 2026-04-29)
> **검증 (Definition of Done)**: 한 펌프 구현체 (`WebClient<T>`) 가
> tokio native + wasm + sync 어댑터 위에서 동일 코드 경로로 동작.
> 게이트웨이/재연결/리다이렉트 포함 §5.5 `justrdp-blocking` feature 100% 재현 + xrdp Docker E2E 양쪽(tokio + wasm) 통과.
> **동기**: M-Client 의 3개 클라이언트 트랙 — `§11.3 web` (브라우저),
> `§11.5a Tauri` (데스크톱 MVP), `§11.5b winit/wgpu` (네이티브) — 이 모두
> async runtime 을 요구함. 현재 `justrdp-blocking` (sync) ↔ `justrdp-web`
> 의 native 사이드 (async) 가 분리된 펌프를 가지고 있어 ~3300 LoC 중복.
> `§11.6 justrdp-gateway server` (M-Server) 의 서버 fan-out 도 thread-per-session 모델 (v1 spawn_blocking) 로는 100+ 세션 호스팅 불가.

#### 5.6 Architecture vision (target end-state)

```
Layer 0 (sans-IO core, 변경 없음):
  justrdp-core, justrdp-pdu, justrdp-connector, justrdp-session, justrdp-input

Layer 1 (NEW: justrdp-async — WebTransport family promoted):
  trait WebTransport               ← justrdp-web 에서 promote
  trait TlsUpgrade<T>              ← justrdp-web 에서 promote
  trait CredsspDriver<T>           ← justrdp-web 에서 promote
  trait GatewayAdapter<T>          ← 신규 (game/web 게이트웨이 추상)
  struct WebClient<T>              ← justrdp-web/driver.rs 에서 promote
  struct ActiveSession<T>          ← justrdp-web/session.rs 에서 promote
  struct Reconnectable<F, T>       ← 신규 (M7 재연결 흡수)
  struct RedirectHandler           ← 신규 (9.3 리다이렉트 흡수)
  InputDatabase                    ← justrdp-blocking 에서 promote (no_std 호환)

Layer 2 (transport implementations):
  justrdp-tokio  : NativeTcp/Tls/Credssp Transport (tokio + tokio-rustls)
  justrdp-web    : WebSocketTransport (wasm32 only)
  justrdp-gateway: TsguHttp/Rpch/Ws Transport (3 variant, async-native 재작성)
  justrdp-blocking: 기존 그대로 두거나 (옵션) sync 어댑터로 슬림화

Layer 3 (high-level clients):
  AsyncRdpClient (tokio, Send+Sync, 게이트웨이/재연결/리다이렉트 포함)
  WasmClient (브라우저)
  RdpClient (sync, 호환용)
```

핵심: **`WebClient<T>` 가 단 하나의 펌프**. 모든 backend 는 transport
trait 만 구현하면 같은 코드 경로 공유. `§11.6 server` 와 `§11.7 proxy` 도
`AcceptorClient<T>` 로 동일 구조 재사용 (M-Server 도입 시).

#### 5.6 — v1 완료 (2026-04-29)

`spawn_blocking` 모델로 임베더 unblock 만 먼저 처리. v1 은 그대로 두고
target end-state 까지 단계적으로 통합.

- [x] `crates/justrdp-tokio/` — `AsyncRdpClient` 가 `spawn_blocking` 으로
      sync `RdpClient` 격리, `mpsc/oneshot` 펌프. connect /
      connect_with_verifier / next_event / send_keyboard / send_unicode
      / send_mouse_{move,button,wheel} / send_synchronize / disconnect
      제공 (commit `e49ad7f`).
- [x] 통합 테스트 3종 (closed-port → ConnectError::Tcp, immediate-close
      handshake → error, `AsyncRdpClient: Send` 컴파일 검증).
- [x] roadmap §5.6.1 trait shape 재검증 — 원래 적힌 `AsyncRead/Write` /
      `AsyncTlsUpgrader` / `AsyncCredsspDriver` 가 모두 잘못된 abstraction
      이라는 결론 (commit `e291df7`). 진짜 abstraction 은 `justrdp-web` 의
      `WebTransport` family — 아래 phase 1 이 그것을 promote.

#### 5.6.1 Phase 1 — `WebTransport` family promote (1주, ~2700 LoC 이동) ✅ **완료 2026-04-29**

- [x] `crates/justrdp-async/` 신설 (`no_std + alloc`, 워크스페이스 등록) (commit `3d602ff`)
- [x] `WebTransport` (`recv()` / `send(&[u8])` / `close()`) trait — message-oriented byte pipe. `justrdp-web/src/transport.rs:31-53` 에서 이동 (commit `1e0ea4e`).
- [x] `TlsUpgrade<T: WebTransport>` trait — `WebTransport → WebTransport` transformer (commit `1e0ea4e`).
- [x] `CredsspDriver<T: WebTransport>` trait — `&mut Connector` + `&mut T` 받아 NLA 단계 진행 (commit `1e0ea4e`).
- [x] `WebClient<T>` 펌프 (~1222 LoC) 와 `ActiveSession<T>` (~557 LoC) 이동 (commit `1e0ea4e`).
- [x] `NativeTcpTransport` / `NativeTlsUpgrade` / `NativeCredsspDriver` 를 `justrdp-web` → `justrdp-tokio` 로 이동 (~960 LoC). `justrdp-web` 은 wasm/렌더 전용 코드 (`canvas.rs`, `js.rs`, `websocket.rs`, `audio.rs`, `clipboard.rs`, `render.rs`, `input.rs`) 만 남음 (commit `498e5f7`).
- [x] 워크스페이스 테스트 통과 — `cargo check --workspace` 그린, `cargo test -p justrdp-async` (27/27), `cargo test -p justrdp-tokio --features native-nla` (21/21), `cargo test -p justrdp-web` (108/108) (`wasm-pack test --headless --firefox` 는 환경 미준비로 보류 — Phase 6 검증과 함께 재실행 예정).
...
| | 시간 | LoC delta |
|---|---|---|
| Phase 1 (WebTransport promote) | 1주 | net 0 (이동만) |
| Phase 2 (feature parity) | 2주 | +1500 |
| Phase 3 (gateway 포팅) | 3주 | +2000, -1540 |
| Phase 4 (`AsyncRdpClient` v2) | 3일 | -200 +400 |
| Phase 5 (옵션 b) | 1주 | -4500 |
| Phase 6 (tests/docs) | 1주 | +500 |
| **합계 (옵션 a)** | **8주** | net +2660 |
| **합계 (옵션 b)** | **9주** | net -1840 |

**위험 / 결정 포인트:**

1. Phase 3 (gateway 포팅) 이 단일 최대 위험. RPCH 의 HTTP/2 chunked 양방향 + NTLM 양방향 협상이 async 환경에서 까다로움. 실제 RD Gateway 테스트 환경 미확보 시 stub 만 작성 가능.
2. `WebClient<T>` async trait 의 `Future + Send` 바운드가 모든 transport 에 전염됨. wasm 은 `!Send` 라 cfg-gated 이중 trait 정의 필요할 수 있음.
3. Phase 5 옵션 (b) 선택 시 `RdpClient` 외부 API 호환성 — 5300 LoC 사용자가 이미 외부에 있을 수 있음. semver bump 필요.
4. InputDatabase 의 no_std 포팅 — 현재 `std::collections::HashSet` 사용 여부 확인. `BTreeSet`/`heapless` 대체 가능하면 OK.

#### 5.6 — Out-of-scope (deferred)

이 phase 에서 의도적으로 제외 — 별도 phase 로 처리:

- ~~`AsyncRead` / `AsyncWrite` trait~~ — rejected 2026-04-29 (잘못된 abstraction, `tokio::io` / `futures-io` 가 표준).
- ~~`AsyncTlsUpgrader` / `AsyncCredsspDriver` standalone trait~~ — rejected 2026-04-29 (`tokio-rustls` + pure no_std `CredsspSequence::step()` 으로 충분, trait 추가 가치 없음).
- 위 거부 사유 상세 — 이 절 이전 commit (`e291df7`) 의 §5.6.1 분석 참조.

---

## 6. Phase 3 -- Standalone Codecs & Primitives  `[M-Client decoders, M-Server encoders]`
...
## 7. Phase 4 -- Session Core & Channel Frameworks  `[both]`
...
## 8. Phase 5 -- Channel Implementations  `[M-Client client-side / M-Server server-side]`
...
## 9. Phase 6 -- Advanced Features & Integration  `[M-Client primary]`
...
## 10. Phase 7 -- Transport Extensions  `[M-Client primary — gateway client / UDP fast-path]`
...
## 11. Phase 8 -- Server-Side & Ecosystem  `[split — see §11.x tags]`

> **목표**: 서버 구현 + Rust 외부 생태계 바인딩.
> Phase 3~6의 클라이언트 구현이 대부분 완료된 후 진행.

### 11.1 `justrdp-acceptor` -- Server Connection Acceptance  `[M-Server]`
...
### 11.2 `justrdp-server` -- Extensible Server Skeleton  `[M-Server]`
...
- [x] `ServerActiveStage::emit_redirection(&ServerRedirectionPdu)`
      API -- Enhanced Security 형식 (MS-RDPBCGR §2.2.13.3.1) 으로
      ShareControl(pduType=0x000A RAW) + pad2 + body 감싸서 I/O 채널
      로 송출. `ShareControlHeader::encode` 의 `to_u16()` (0x001A
      산출) 를 우회하고 raw 0x000A 직접 write
- [x] PDU 옵션 전체 경로 지원 -- 11 개 data 필드 (TargetNetAddress,
      LoadBalanceInfo, UserName, Domain, Password, TargetFQDN,
      TargetNetBiosName, TargetNetAddresses, TsvUrl, RedirectionGuid,
      TargetCertificate) + 5 개 modifier 비트 (NOREDIRECT,
      SMARTCARD_LOGON, DONTSTOREUSERNAME, SERVER_TSV_CAPABLE,
      PASSWORD_IS_PK_ENCRYPTED) passthrough. `Encode` impl 에서 flag
      ↔ Option 필드 일관성 자동 검증 + PK_ENCRYPTED without PASSWORD
      거부
- [x] Emit 후 상태 처리 -- `has_emitted_redirection: bool` one-shot
      flag, 두 번째 `emit_redirection` 호출 시 protocol error;
      `Deactivation-Reactivation` 진행 중엔 emit 차단; 실패한 emit
      은 flag flip 하지 않음
- [x] 단위 테스트 (13건: 7 개 PDU 레이어 roundtrip / 경계값 / 거부
      + 6 개 서버 레이어 wire 포맷 / pduType=0x0A / one-shot /
      DR state gating / totalLength / channel_id=io_channel_id 검증)
- [x] 통합 테스트 -- `handshake_loopback.rs::
      server_emit_redirection_during_finalization_reaches_connected`
      -- acceptor 가 `Accepted` 도달하는 순간 s2c 의 FontMap 을
      redirection 프레임으로 교체 → client 가 파싱 후 `Connected` 로
      전이 + `ConnectionResult.server_redirection` 필드에 원본 PDU
      복원 확인
- [x] Standard Security (PROTOCOL_RDP + RC4) 변형은 §11.2a-stdsec
      (Appendix G.2) 완료 후 추가; 현재는 Enhanced Security 경로만
      (ClientConnector 의 finalization 루프가 인식하는 유일한 형식)
...
- [x] Caller-managed RNG trait 주입 -- `RandomSource::fill_random(&mut
      self, buf: &mut [u8])` (`justrdp-server/src/random.rs`); crate 가
      RNG 선택을 강제하지 않음
- [x] `ServerActiveStage::emit_save_session_info(data:
      SaveSessionInfoData)` generic API -- `SaveSessionInfoData` enum
      (LogonV1 / LogonV2 / PlainNotify / Extended) 로 4 가지 infoType
      모두 커버. `LogonInfoV1` / `LogonInfoV2` / `LogonInfoExtended` /
      `ArcScPrivatePacket` 는 server crate 에서 re-export
- [x] `ServerActiveStage::emit_auto_reconnect_cookie(logon_id, rng)`
      편의 API -- 16B ArcRandomBits 생성 (`ArcScPrivatePacket`) +
      `LogonInfoExtended` wrap + `INFOTYPE_LOGON_EXTENDED_INFO` 인코드
      + 송출 + `(frame_bytes, cookie)` 반환. One-shot 아님 -- §5.5
      정책대로 재호출 시 새 cookie 로 교체
- [x] Cookie retention hook -- `current_auto_reconnect_cookie()` 로
      현재 cookie 조회. Stage 는 단일 세션의 최신 cookie 만 보관
      (외부 저장소 정책은 caller 몫; in-memory / Redis / DB)
- [x] Guard 조건 -- `emit_redirection` 이후 / Deactivation-Reactivation
      중이면 `ServerError::protocol(_)`; 거절된 호출은 state 변경 없음
- [x] `verify_auto_reconnect_packet(stored: &ArcScPrivatePacket,
      received: &ArcCsPrivatePacket, client_random: &[u8; 32]) ->
      Result<(), ArcVerifyError>` + `ENHANCED_SECURITY_CLIENT_RANDOM`
      상수 (`justrdp-server/src/arc_verify.rs`). HMAC-MD5 constant-
      time compare (`ct_eq_16`), logon_id short-circuit, 8건 단위
      테스트 (positive Standard/Enhanced, 공격자 시나리오 4건 포함
      all-zero / wrong bits / 앞뒤 bit flip, logon_id 불일치,
      Debug/Eq identity). 완전한 재인증 경로 (Security Exchange →
      세션 키 재개) 는 `§11.2a-stdsec` scope
- [x] 단위 테스트 (7건: PlainNotify roundtrip, Extended+ARC emit
      (RNG → 와이어 바이트 보존 + `arc_random()` 경유 검증), 재-emit
      시 current_cookie 교체, redirection 이후 차단, D/R 중 차단,
      logon_id 경계값 roundtrip 0 / 1 / `u32::MAX`, server crate
      re-export 와 PDU crate `ArcScPrivatePacket` 타입 identity)
- [x] 통합 테스트 -- `handshake_loopback.rs::server_emit_auto_reconnect_cookie_reaches_client_connection_result`
      서버 emit → FontMap 앞에 splice → 실제 `ClientConnector` 가
      `ConnectionResult.server_arc_cookie` 에 logon_id + arc_random_bits
      복원

**진행**: Commit 1 (emit API + 단위 7건 + 통합 1건) + Commit 2
(verify helper + 단위 8건) 완료. §11.2f 종료.

### 11.3 `justrdp-web` -- WASM Bindings  `[M-Client]`

> **requires**: Phase 3 코덱 (`no_std`), Phase 4/5 세션+채널

> **아키텍처 원칙**: `justrdp-web` 은 라이브러리. mstsc 직결 외에 자체 게이트웨이/Tauri/WebTransport/WebRTC 등 다양한 임베딩을 지원하기 위해 **transport-agnostic core + 레퍼런스 구현** 분리. 모든 상위 레이어 (`WasmConnector`, 렌더러, 입력, 채널) 는 `T: WebTransport` 로 generic.
>
> **Tauri / native 임베딩 경로**: 현재 검증된 타깃은 `wasm32-unknown-unknown` +
> `WebSocketTransport` 한 조합 뿐이다. 데스크톱 Tauri 앱에서 같은 코어를
> 재사용하려면 §11.3 S7 (Native Embed Validation) 을 통해
> `NativeTcpTransport` + `NativeTlsUpgrade` + native NLA 어댑터 결선이
> 선행되어야 한다. 그 위에 §11.5a Tauri Reference Binary 가 GUI 를 얹는다.

#### S1 — 골격 + Transport trait + WebSocket 레퍼런스 (완료)
- [x] crate 스켈레톤 (`crates/justrdp-web/`, workspace 등록, `no_std` + `extern crate alloc` + `forbid(unsafe_code)`)
- [x] `WebTransport` trait — `async fn send/recv/close`, transport-agnostic. cancel-safe / 메시지 단위 framing 계약 명시
- [x] `TransportError` + `TransportErrorKind` (`ConnectionClosed`/`Io`/`Protocol`/`Cancelled`/`Other`) — `core::error::Error` 구현
- [x] 레퍼런스 `WebSocketTransport` (wasm32 + `feature = "websocket"` 게이트, `web_sys::WebSocket` 어댑터). 단일 스레드 `Rc<RefCell<Inner>>` + `Waker` 모델로 onmessage→async recv 브릿지
- [x] `WebSocketConfig` (url + 서브프로토콜 옵션)
- [x] `MockTransport` 단위 테스트 (`#[cfg(test)]` 한정, 5건) + trait generic 컴파일 가드 (1건) + error 단위 (3건). 9건 모두 그린
- [x] `cargo build` (native) / `cargo build --target wasm32-unknown-unknown` 양쪽 그린

#### S2 — Connection driver + JS facade (Standard Security) (완료)
- [x] `WebClient<T: WebTransport>` 드라이버 — `ClientConnector` 상태 머신을 `Connected` 까지 비동기 펌프. send/recv/hint 루프 `justrdp-blocking::pump_connector` 와 동치, transport-agnostic
- [x] `recv_until_pdu` 재프레이밍 — 브릿지가 PDU 단위로 끊지 않더라도 `PduHint` 로 안전 재조립, `MAX_HANDSHAKE_PDU_SIZE = 16 MiB` 가드
- [x] `DriverError` (Transport / Connector / FrameTooLarge / TlsRequired / NlaRequired / Internal). 미지원 상태 (`EnhancedSecurityUpgrade`, CredSSP/AAD/RDSTLS) 는 typed error 로 조기 탈출
- [x] `wasm-bindgen` 기반 JS facade `justrdpConnect(url, user, pw, domain?)` — Promise 반환, `WebSocketTransport` + Standard Security 종단 연결 데모. resolve 시 `{shareId, ioChannelId, userChannelId, channels[], selectedProtocol}` JS 객체
- [x] 브라우저 RNG 통합 (`getrandom` `js` feature) — `Config.client_random` 자동 채움
- [x] 단위 7건 추가 (driver error mapping / recv 재프레이밍 / oversize 거부 / 빈 frame 거부 / X.224 CR 송출 + EOF 전파 / SSL 선택 시 `TlsRequired`). 양쪽 타깃 그린

#### S3a — 활성 세션 펌프 (완료)
- [x] `ActiveSession<T: WebTransport>` — `ConnectionResult` + `WebTransport` 묶어 post-handshake 펌프. `next_events()` 은 한 프레임을 읽어 `ActiveStage` 로 처리, `ResponseFrame` 은 자동 재송출, 유저 가시 사건만 `Vec<SessionEvent>` 로 surface
- [x] `SessionEvent` 추상화 — `Graphics` / `Pointer` / `Channel` / `Reactivation` / `SaveSessionInfo` / `MonitorLayout` / `KeyboardIndicators` / `KeyboardImeStatus` / `PlaySound` / `SuppressOutput` / `Terminated`. ActiveStage 전 출력 변종을 1:1 매핑
- [x] `send_input` / `shutdown` / `disconnect` — fast-path input, Shutdown Request, MCS DisconnectProviderUltimatum 송출
- [x] `DriverError::Session(SessionError)` 변형 추가, `recv_until_pdu` 를 `pub(crate)` 로 공유
- [x] 단위 4건 추가 (transport EOF 전파 / scratch 재사용 / `into_transport` ownership 회수 / 최소 fast-path Synchronize 프레임 처리). 워크스페이스 3828/3828

#### S3b — FrameSink trait + Canvas 2D 레퍼런스 + bitmap fast-path 디코드 (완료)
- [x] `FrameSink` trait — `resize` (default no-op) + `blit_rgba(x, y, w, h, &[u8])` + `flush` (default no-op). transport-agnostic 와 같은 철학으로 렌더 백엔드도 자유 교체
- [x] `decode_bitmap_update_fast_path(payload) -> Vec<DecodedRect>` — `TS_UPDATE_BITMAP_DATA` 디코더, BGRA 보텀업 → RGBA 톱다운 단일 패스 변환 (Canvas/WebGL/wgpu 가 직접 받을 수 있는 포맷)
- [x] `RenderError` (`Decode` / `Unsupported` / `CompressedNotSupported` / `UnsupportedBpp` / `SizeMismatch`) — 미지원 변환은 모두 typed error 로 surface. 압축 (RLE), 16/24/15 bpp, Orders 등은 후속 (S3c)
- [x] `render_event(event, &mut sink)` 디스패처 — `SessionEvent::Graphics{Bitmap}` 처리, Synchronize/SurfaceCommands 무시 (no-op), Pointer/Channel 등 비-그래픽 이벤트 `false` 반환
- [x] `CanvasFrameSink` (wasm32-only, `from_canvas` / `from_context` 두 진입점) — `ImageData::new_with_u8_clamped_array_and_sh` + `put_image_data`. scratch 버퍼 재사용, `auto_resize` 토글 (default on)
- [x] 단위 7건 추가 (32bpp 디코드 + B↔R 스왑 + 행 플립 동시 검증 / 압축 거부 / 비-32bpp 거부 / Bitmap 디스패처 / Synchronize 무시 / 비-그래픽 이벤트 / 미지원 update_code). 워크스페이스 3835/3835

#### S3c — RLE / 색공간 / JsClient / 데모 (완료)
- [x] **S3c-A** RLE 압축 디코드 — `BITMAP_COMPRESSION` 플래그 시 `justrdp_graphics::RleDecompressor` 경유, 그 후 per-bpp 변환기로. `RenderError::CompressedNotSupported` 제거, `RenderError::Rle(RleError)` 신설
- [x] **S3c-A** 색공간 변환 (single pass flip+convert): 32 bpp BGRA, 24 bpp BGR, 16 bpp RGB565 (5/6 bit replication), 15 bpp RGB555 → 모두 RGBA 톱다운. 8 bpp 만 거부 (Palette PDU 미지원, S3d 잔여)
- [x] **S3c-B** stateful `JsClient` (wasm-bindgen) — `Rc<RefCell<Inner>>` + take/put-back 패턴으로 await 사이 borrow 미보유. 메서드: `new()` / `attachCanvas(canvas)` / `connect(url, user, pw, domain?)` / `pollEvents() -> blit count` / `disconnect()` / `connected` getter / `lastSummary` getter. `Terminated` 이벤트 자동 detach
- [x] **S3c-B** 데모 페이지 — `crates/justrdp-web/examples/demo/{index.html, main.js}`. Standard Security 직결, wsproxy 류 투명 TCP-over-WS 브릿지 가정. wasm-pack 빌드 명령은 `index.html` 헤더 주석에 명시
- [x] 단위 +6 (24/16/15 bpp 직접 변환 / RGB565 비트 복제 / RLE WHITE special order 라운드트립 / RLE 에러 surface). 워크스페이스 3839/3839, wasm32 빌드 그린

#### S3d-1 — BitmapRenderer + Palette PDU + 8 bpp indexed (완료)
- [x] `BitmapRenderer` (stateful) — 팔레트 캐시를 보유, `render(event, &mut sink)` 가 update_code 분기. JsClient 가 보유해 세션 간에는 새 인스턴스, 동일 세션 내에서는 팔레트 유지
- [x] Palette PDU 디코드 (`TS_FP_UPDATE_PALETTE`: pad2(2) + numberColors(4) + 256×3 RGB). `numberColors != 256` / 본문 truncation 둘 다 typed `RenderError::SizeMismatch`
- [x] 8 bpp uncompressed + RLE 경로 — `BitsPerPixel::Bpp8` 로 RleDecompressor 통과 후 `flip_and_apply_palette` 단일 패스 (행 플립 + 팔레트 룩업 + 알파 0xFF 픽스)
- [x] `RenderError::PaletteMissing` 신설 — 8 bpp 비트맵이 Palette 없이 도착 시 surface (stateless `decode_bitmap_update_fast_path` 도 동일 에러)
- [x] 단위 +5 (팔레트 캐시 후 8bpp 디코드 / 팔레트 없는 8bpp / RLE 8bpp / 잘림 팔레트 / 잘못된 numberColors). 단위 36/36, 워크스페이스 3844/3844
- [x] **S3d 이후** WebSocket 전송 (브라우저 환경) — bpp 8/15/16/24/32 모든 경로 + 압축/비압축 양쪽 처리
#### S3d-2 — Surface Commands (완료)
- [x] `BitmapRenderer::process_surface_commands` — `FASTPATH_UPDATETYPE_SURFCMDS` 페이로드 워크. `SurfaceCommand::decode` 루프로 잔여 바이트 0 까지 디스패치
- [x] `SET_SURFACE_BITS` / `STREAM_SURFACE_BITS` 의 `codec_id == 0` (raw) 경로 — 32 bpp BGRA top-down → RGBA blit (legacy Bitmap 과 달리 행 플립 없음, MS-RDPBCGR 2.2.9.2.1.2.1.1)
- [x] `FRAME_MARKER` — `last_frame_id` 캐시, `END` action 시 `sink.flush()` 호출 (per-frame 커밋 hook)
- [x] `RenderError::UnsupportedCodec { codec_id }` — RFX/NSCodec/ClearCodec/AVC 등 비-제로 codec_id 는 typed error 로 surface (S3d-3+ 까지 보류)
- [x] 단위 +3 (raw 32bpp BGRA → RGBA blit / FRAME_MARKER BEGIN+END 시퀀스 + flush 횟수 / 미지원 codec_id 0x03 거부). 단위 39/39, 워크스페이스 3847/3847
#### S3d-3 — RemoteFX 디코드 + Surface Commands 연동 (완료)
- [x] `BitmapRenderer.set_rfx_codec_id(u8)` — 서버가 capability negotiation 시 임의로 할당하는 codec_id 를 embedder 가 설정. 미설정 시 RFX 페이로드도 `UnsupportedCodec` 으로 surface (안전 가드)
- [x] `decode_rfx_stream` — `WBT_SYNC` / `WBT_CODEC_VERSIONS` / `WBT_CHANNELS` / `WBT_CONTEXT` / `WBT_FRAME_BEGIN` / `WBT_REGION` / `WBT_EXTENSION` (TileSet) / `WBT_FRAME_END` 블록 시퀀스 워크. image-mode (frame 마다 handshake) + video-mode (handshake 1회) 양쪽 호환 — Context 블록은 마주칠 때마다 `rfx_entropy` 갱신
- [x] `blit_rfx_tileset` — `RfxDecoder::decode_tile` 통과 (RLGR → Subband → Dequantize → IDWT → YCbCr→BGRA). 출력 BGRA → RGBA 단일 swap, 위치 = `(dest_left + x_idx*64, dest_top + y_idx*64)` saturating_add 보호
- [x] `RenderError::Rfx(RfxError)` + `RfxQuantIndexOutOfRange { quant_idx, num_quants }` — RLGR/DWT 실패와 깨진 TileSet 둘 다 typed error
- [x] 단위 +2 (server-side `RfxFrameEncoder` 로 image-mode 1-tile frame 생성 → BitmapRenderer 디코드 round-trip + 위치 / 크기 / 알파 검증 / `rfx_entropy` 갱신 / RFX codec_id 미등록 시 `UnsupportedCodec`). 단위 41/41, 워크스페이스 3849/3849
#### S3d-4 — Drawing Orders 워커 + OpaqueRect 렌더 (완료)
- [x] `BitmapRenderer.process_orders` — `numberOrders (u16) + N orders`. controlFlags 첫 비트로 분류:
  - `TS_STANDARD` clear → Alternate Secondary (orderLength prefix 로 안전 skip)
  - `TS_STANDARD | TS_SECONDARY` → Secondary (orderLength + extraFlags + orderType + body 안전 skip)
  - `TS_STANDARD` only → Primary 워커 진입
- [x] Primary 워커 — `ORDER_TYPE_CHANGE` 분기 + `last_primary_type` 캐시 (type-change 절약 처리), `TS_ZERO_FIELD_BYTE_BIT0/1` 카운트로 fieldFlags 바이트 절약 디코드, `TS_BOUNDS` (`TS_ZERO_BOUNDS_DELTAS` 무인코딩) 분기, `TS_DELTA_COORDINATES` 전달
- [x] OpaqueRect 렌더 — `decode_opaque_rect` 결과로 단색 사각형 RGBA 채워 `sink.blit_rgba`. width/height 음수 방어 클램프
- [x] DstBlt / PatBlt / ScrBlt / MemBlt / LineTo — 디코더만 호출 (필드 history 갱신 + 커서 진행), 렌더 미지원. 미식별 Primary 타입은 `UnsupportedPrimaryOrder` typed error (해당 후 stream 파괴 가능 — embedder 가 결정)
- [x] `RenderError::UnknownOrderClass` / `UnsupportedPrimaryOrder` 신설
- [x] `BitmapRenderer.primary_history: PrimaryOrderHistory` + `last_primary_type` 필드 — 세션 동안 delta encoding / type-change 의 정확성 보장
- [x] 단위 +3 (OpaqueRect single-color blit / numberOrders=0 noop / 미지원 SaveBitmap typed error). 단위 55/55, 워크스페이스 3863/3863
#### S3d-5 — NSCodec + ClearCodec + AVC420 / AVC444 / AVC444v2 코덱 통합 (완료)
- [x] **NSCodec** (MS-RDPNSC) — `BitmapRenderer.set_nscodec_codec_id(u8)`. dispatch 시 `NsCodecDecompressor::decompress` → 32 bpp BGRA → RGBA top-down blit (Surface Commands top-down 규약, 행 플립 없음)
- [x] **ClearCodec** (MS-RDPEGFX §2.2.4) — `BitmapRenderer.set_clearcodec_codec_id(u8)`. 첫 등록 시 `ClearCodecDecoder` lazy-alloc, glyph + VBar 캐시 세션 내 영속. dispatch 시 BGR (24 bpp) → RGBA (alpha 0xFF 핀)
- [x] `RenderError::NsCodec(NsCodecError)` / `ClearCodec(ClearCodecError)` typed errors. 미등록 codec_id 는 기존처럼 `UnsupportedCodec` 으로 surface
- [x] `BitmapRenderer` 의 `derive(Debug, Clone)` 제거 (ClearCodecDecoder 가 Debug/Clone 미구현). 수동 Debug 가 codec 내부 캐시는 redact 해 finish_non_exhaustive 사용
- [x] 단위 +5 (NSCodec dispatch routes via codec error / NSCodec 미등록 → UnsupportedCodec / ClearCodec dispatch / ClearCodec 미등록 / 등록 시 lazy-alloc + Debug redact). 단위 60/60, 워크스페이스 3868/3868
#### S3d-5b/5c — AVC420 / AVC444 / AVC444v2 dispatch (완료)
- [x] `BitmapRenderer.set_avc_decoder(Box<dyn AvcDecoder>)` / `clear_avc_decoder()` / `has_avc_decoder()` — embedder 가 H.264 디코더 주입 (browser `VideoDecoder` / openh264 / FFmpeg / HW backend 등). justrdp-web 자체는 H.264 미동봉
- [x] `set_avc420_codec_id(u8)` / `set_avc444_codec_id(u8)` / `set_avc444v2_codec_id(u8)` — 서버 capability negotiation 결과 codec_id 등록 (AVC444 v1 / v2 분리)
- [x] **AVC420 wire 파서** (`RDPGFX_AVC420_BITMAP_STREAM`, MS-RDPEGFX 2.2.4.4.1) — `numRegionRects` (u32 LE) + 8 byte/region rects + 3 byte/quantQualityVals + Annex B trailer. metablock 분리 후 trailer 만 디코더에 투입
- [x] **AVC420** dispatch — `Yuv420Frame` → `yuv420_to_bgra` (BT.709 full-range) → BGRA → RGBA → blit. 16-aligned 코딩 사이즈 vs 디스플레이 사이즈 차이 처리
- [x] **AVC444 envelope 파서** (`RDPGFX_AVC444_BITMAP_STREAM`, MS-RDPEGFX 2.2.4.4.2/.3) — `cbAvc420EncodedBitstream1` (high 4 bits = LC, low 28 bits = bitstream1 size) + bitstream1 + optional bitstream2
- [x] **AVC444 LC=0** → main view 단독 (AVC420 fallback 경로); **LC=2** → main + aux 동시 디코드 → `combine_avc444_planes` (v1) / `combine_avc444v2_planes` (v2) 분기 → `yuv444_to_bgra` → blit; **LC=1** → typed `AvcLumaCacheRequired` (이전 frame luma cache 미보유)
- [x] `RenderError::Avc(AvcError)` / `AvcDecoderMissing` / `AvcFrameUnavailable` / `AvcLumaCacheRequired` typed errors. 미등록 codec_id 는 기존처럼 `UnsupportedCodec`
- [x] 단위 +8 누적 (AVC420 round-trip / 미주입 typed missing / decoder None / clear disarm / AVC444 LC=0 / AVC444 LC=2 v1 layout 합성 / AVC444 LC=1 luma-cache typed / AVC444v2 dispatch). 단위 68/68, 워크스페이스 3876/3876
#### S3d-6 — 추가 Primary Drawing Order 렌더 (부분 완료, 더 많은 cases 처리)
- [x] **PatBlt** with `BS_SOLID` brush + `PATCOPY` ROP — fore_color 채운 사각형 blit (S3d-6a)
- [x] **DstBlt** `BLACKNESS` (0x00) / `WHITENESS` (0xFF) ROP — 단색 fill. `DSTINVERT` 등 destination read-back 필요한 ROP 은 silent skip (FrameSink 가 write-only) (S3d-6f)
- [x] **LineTo** — Bresenham 단일 픽셀 라인, pen_color 사용. `pen_width` ≥ 2 / dashed pen_style / `back_color`-with-OPAQUE back_mode 는 무시 (S3d-6e)
- [x] 단위 +3 (DstBlt BLACKNESS 라운드트립 + DSTINVERT silent skip + LineTo horizontal 5-pixel). 단위 74/74, 워크스페이스 3882/3882
- [x] **PatBlt with `BS_PATTERN`** (8×8 인라인 모노크롬) — `brush_hatch` (top row) + `brush_extra` (rows 1-7) 인라인 패턴 + brush_org 위상. PATCOPY 는 read-back 없이, PATPAINT/PATINVERT 등 RMW ROP 은 ROP3 evaluator + `peek_rgba` 통과
- [x] **PatBlt `BS_HATCHED`** — 6 predefined 8×8 hatch (HS_HORIZONTAL/VERTICAL/FDIAGONAL/BDIAGONAL/CROSS/DIAGCROSS), `brush_hatch` 인덱스 (S3d-6b2)
- [x] **CacheBrush 1bpp + PatBlt cached** — `brush_style` high-bit-set + `brush_hatch` 가 cache index. CacheBrush V1 (orderType 0x07) 1bpp 8×8 파서 (S3d-6b2)
- [x] **ROP3 evaluator** — 8-bit truth table (idx = `P<<2 | S<<1 | D`) per channel. SRCCOPY/SRCAND/SRCPAINT/SRCINVERT/MERGEPAINT/PATPAINT/PATINVERT/DSTINVERT/BLACKNESS/WHITENESS 모두 자동 처리 (S3d-6c4 / 6b ROP path)
- [x] **MemBlt RMW ROPs** — SRCCOPY 외 codes 는 `peek_rgba` 로 destination 읽고 ROP3 evaluator 통과. peek 미지원 sink 는 silent skip. 단위 +1 (rop3 truth table) (S3d-6c4)
- [x] **GlyphIndex skeleton** — 22-field body 인라인 walker. cursor 정상 진행, 렌더 미구현 (텍스트 렌더는 `CacheGlyph` 파서 + 1bpp 마스크 합성 = 별도 큰 작업 → S3d-6d2)
- [x] **PolygonSC / PolygonCB** — 인라인 디코더 (xStart/yStart + brush color + nDeltaEntries + delta 리스트, CB 변종은 brush 6 필드 추가). 정점 복원 후 even-odd scanline fill 로 brush_color 채움. 단위 +1 (triangle SC fill verifies brush color RGBA on every span). CB 의 brush pattern 자체는 미렌더 (color 만) (S3d-6g2)
- [x] **CacheBitmapV2 24 bpp** — uncompressed (BGR top-down → RGBA) + RLE (`Bpp24` 디코더 통과). 단위 +1 (Polygon test 가 함께 있어 워크스페이스 +4 = 84 단위) (S3d-6c3 일부)
- [x] **CacheBitmapV1** (8/16/24 bpp uncompressed + RLE) — 9-byte 고정 헤더 + bottom-up DIB → top-down RGBA flip + 변환. 8 bpp 는 Palette PDU 가 먼저 수신된 경우 `flip_and_apply_palette` 로 indexed → RGBA 변환, 미설정 시 silent drop (S3d-6c5 + 8 bpp 분기 S3d-6c5c)
- [x] **CacheBitmapV3** (codec_id=0 raw 32/24 bpp + codec-encoded) — TS_BITMAP_DATA_EX 헤더 + 옵션 EX 헤더 + 페이로드. codec_id != 0 인 경우 `BufferSink` 로 fast-path Surface Bits 디스패치 (`dispatch_surface_bits`) 를 캐시 적재 경로로 재사용 → RFX/NSCodec/ClearCodec/AVC420/AVC444/AVC444v2 모두 V3 캐시 진입 가능 (S3d-6c5 + S3d-6c5b)
- [x] **MemBlt + CacheBitmapV2 (32 bpp uncompressed)** — Secondary `CacheBitmapV2Uncompressed` 파서 (`extra_flags` 의 cacheId / bppCode + `TWO_BYTE_UNSIGNED` width/height/cacheIndex + `FOUR_BYTE_UNSIGNED` length + 픽셀 데이터). `BTreeMap<(cache_id, cache_index), CachedBitmap>` 캐시 보유. MemBlt SRCCOPY (0xCC) 시 cache 룩업 → src_left/src_top crop → blit. cache miss / non-32bpp / non-SRCCOPY 는 silent skip
- [x] 단위 +2 (CacheBitmapV2 → MemBlt round-trip 4×2 → 3×2 sub-region crop / cache miss silent skip). 단위 76/76, 워크스페이스 3884/3884
- [x] **CacheBitmapV2Compressed (16 bpp RLE)** — `RleDecompressor` (Bpp16) → `rgb565_top_down_to_rgba` 통과해 캐시 적재. 시크릿: V2 캐시 엔트리는 spec 상 top-down 이라 행 플립 없음. 32 bpp 압축은 spec 미정의 (V3 영역) (S3d-6c2)
- [x] **`FrameSink::peek_rgba`** trait 메서드 추가 — `(x,y,w,h, &mut out)` → `bool`. 디폴트 `false`. read-back 안 하는 sink 는 RMW ROP blit 자동 silent drop (display 망가지지 않음)
- [x] **DstBlt DSTINVERT (0x55)** — `peek_rgba` 로 destination 읽기 → R/G/B 인버트 (alpha 보존) → 다시 blit (S3d-6h)
- [x] **Polyline** 인라인 디코더 + Bresenham — `xStart/yStart` (i16/i16) + `bRop2/BrushCacheEntry` skip + pen color (BGR) + numDeltaEntries + `cbData(u16)` + delta 리스트 (TWO_BYTE_SIGNED). `draw_bresenham_segment` helper 가 `LineTo` 와 공유 (S3d-6g)
- [x] 단위 +4 (CacheBitmapV2Compressed RLE 16bpp 캐시 / Polyline 2-segment + pen color RGBA / DSTINVERT seeded peek + 인버트 / DSTINVERT peek 미지원 silent drop). 단위 80/80, 워크스페이스 3888/3888
- [x] **CacheBitmapV2 8 bpp** (uncompressed + RLE) — `bpp_code = 0x03` 분기 추가, 새 `apply_palette_top_down` 헬퍼로 indexed → RGBA 변환 (V2 cache 는 spec 상 top-down 이라 행 플립 없음). Palette PDU 미수신 시 silent drop (V1 8 bpp 와 동일 페일 모드). V1 8/16/24 + V2 24/16 + V3 32/24/codec 은 이전 단계에서 완료됨. 단위 +3 (V2 uncompressed 8 bpp + 팔레트 → MemBlt round-trip / 팔레트 없을 때 silent skip / V2 RLE 8 bpp + 팔레트). 단위 116/116, 워크스페이스 그린, wasm32 그린 (S3d-6c3)
- [x] **MemBlt 기타 ROP** — `try_render_memblt` 가 SRCCOPY (0xCC) fast path 외에 `peek_rgba` 로 destination 을 읽어 들여 `rop3(rop, P=0xFF, S, D)` 평가 후 다시 blit. P=0xFF 는 MemBlt 가 패턴 없음을 의미 (`render.rs:1638-1662`). `peek_rgba` 미지원 sink 는 silent drop (S3d-6c4)
- [x] **Polygon (P/CB)** — `try_render_polygon_sc` (`render.rs:2797`) 가 even-odd `scanline_fill_polygon` 으로 brush_color 채움. `try_render_polygon_cb` (`render.rs:2833`) 는 BS_HATCHED / BS_PATTERN (인라인 + 캐시 양쪽) brush 를 `scanline_fill_polygon_brush` 로 픽셀별 fore/back 합성. 단위 +1 (S3d-6g2 / 6g2-pattern 통합)
- [x] **GlyphIndex 텍스트 렌더** — `CacheGlyph V1` 파서 (orderType 0x03, cacheIndex/cx/cy/1bpp mask × cGlyphs entries) + glyph cache + GlyphIndex 22-field 본체 디코더 + cbData 명령 스트림 워커 (single index / 0xFE cache-id switch / 0xFF terminator). `composite_glyph_mask` 가 1bpp 마스크 → fore_color RGBA 합성 (S3d-6d2)
- [x] **GlyphIndex bk rect** — `BkLeft..BkBottom` 비-empty 시 `BackColor` RGBA fill 을 glyph 합성 *전에* blit. 고립된 텍스트 스팬도 정상 렌더 (S3d-6d3)
- [x] **flAccel SO_VERTICAL / SO_REVERSED** — 활성 축이 vertical 시 pen 진행을 y 축으로, reversed 비트 시 양수 → 음수 진행 + 사전 origin shift (right-edge / bottom 정렬). horizontal 좌→우 가 default (S3d-6d3)
- [x] **`GlyphCacheRevision` enum + `set_glyph_cache_revision`** — 서버가 negotiate 한 V1/V2 revision (default V1) 을 embedder 가 셋 (S3d-6d3)
- [x] **CacheGlyph V2 wire 파서** — `extra_flags` 가 cacheId(4)/flags(4)/cGlyphs(8) 를 패킹, body 는 entries × {cacheIndex/x/y/cx/cy 모두 `TWO_BYTE_UNSIGNED` + 4-byte 정렬 1bpp 마스크}. V1 과 같은 `glyph_cache` 슬롯에 적재 → GlyphIndex 렌더 경로 그대로 재사용 (S3d-6d4)
- [x] **Polyline + Polygon (SC + CB)** — Bresenham (Polyline) + scan-line even-odd fill (PolygonSC solid color, PolygonCB textured). 디코더는 `justrdp-pdu` 의 `decode_polyline / decode_polygon_sc / decode_polygon_cb` 로 upstream (`PolylineOrder / PolygonScOrder / PolygonCbOrder` typed structs + `DeltaPoint` 공용) (S3d-6g3)
- [x] **PolygonCB textured fill** — `BS_HATCHED` / `BS_PATTERN` (인라인 + 캐시 양쪽) + brush_org 가 PolygonCB scan-line 에 직접 적용. `scanline_fill_polygon_brush` 가 픽셀별 fore/back 색 결정 후 한 span 당 한 blit (S3d-6g2-pattern)
- [x] **DstBlt 나머지 ROP / DSTINVERT** — `try_render_dstblt` (`render.rs:3087-3120`) 가 DSTINVERT (0x55, `peek_rgba` 기반 RGB 인버트), BLACKNESS (0x00 → 0,0,0), WHITENESS (0xFF → 255,255,255) 처리. 0xAA NOP 는 silent skip — DstBlt 의 wire-spec 상 valid D-only ROP 4종은 모두 커버 (S3d-6h)

#### S3-NLA — TLS upgrade pass-through (외부 브릿지) (완료)
- [x] `WebClient::with_external_tls(bool)` builder + `external_tls()` getter — `EnhancedSecurityUpgrade` 상태를 "브릿지가 이미 TLS 종단했음" 으로 가정. step 무시 + 다음 상태로 진행 (SSL → BasicSettingsExchange, HYBRID → CredsspNegoTokens 그 다음 NLA 미지원 통해 typed `NlaRequired` 으로 빠짐)
- [x] `JsClient::setExternalTls(boolean)` — JS-side 토글. 데모 페이지 (`index.html`) 에 체크박스 + `main.js` 가 `client.setExternalTls(...)` 호출
- [x] 단위 +2 (with_external_tls(true) → SSL upgrade 통과 + 추가 send 발생 / default false → 기존 TlsRequired 보존). 단위 71/71, 워크스페이스 3879/3879
- [x] **In-band TLS handshake** — embedder-injected `TlsUpgrade` trait (associated `Output: WebTransport` + `Error: Display`) + `WebClient::connect_with_upgrade(config, upgrader)` 메서드. 3-phase pump (pre-TLS / 업그레이드 / post-TLS) 가 `pump_until_terminal` 헬퍼 공유. 서버가 SSL 선택 시 자동 업그레이드, Standard Security 선택 시 typed `Internal` 으로 surface. justrdp-web 자체는 TLS 미동봉 (native: `justrdp-tls::RustlsUpgrader` 어댑터, browser: `with_external_tls(true)` + wss://) (S3-NLA-2)
- [x] **CredSSP / NLA** (embedder-injected) — `CredsspDriver<T: WebTransport>` trait + `WebClient::connect_with_nla(config, tls, credssp)` 5-phase pump (pre-TLS / TLS upgrade / mid pump / CredSSP / post-CredSSP). justrdp-web 자체는 SPNEGO/NTLM/Kerberos 미동봉 — native 는 `justrdp-connector::CredsspSequence` + 플랫폼 NTLM/Kerberos (Windows SSPI / libkrb5) 어댑터, browser 는 일반적으로 NLA 미지원 (브릿지에서 pre-auth). `DriverError::Credssp(String)` typed wrapper (S3-NLA-3)
#### S4 — 입력 forwarding (브라우저 → fast-path PDU) (완료)
- [x] `input` 모듈 — `scancode_event` / `mouse_move_event` / `mouse_button_event` / `mouse_wheel_event` 순수 함수 (transport-agnostic). PS/2 set-1 scancode + KBDFLAGS_RELEASE / EXTENDED 조합, PTRFLAGS_BUTTONn / DOWN / WHEEL / HWHEEL / WHEEL_NEGATIVE 비트 합성. wheel delta 는 i8 클램프 + sign 분리
- [x] `MouseButton` enum (`Left` / `Right` / `Middle`) — DOM `MouseEvent.button` 직접 매핑은 JsClient 가 수행
- [x] `JsClient::sendKeyDown(scancode, extended)` / `sendKeyUp` / `sendMouseMove(x, y)` / `sendMouseButton(x, y, button, pressed)` / `sendMouseWheel(x, y, delta, horizontal)` — 모두 take/put-back 패턴, await 가로지르는 borrow 미보유. session 미연결 시 `"not connected"` typed error
- [x] 데모 페이지 — `keymap.js` (US-English `KeyboardEvent.code` → PS/2 set-1 ~70개 매핑, 확장 키 플래그 포함). `main.js` 가 canvas focus / mousemove / mousedown,up / wheel / contextmenu / keydown,up 리스너를 connect 후 첨부, disconnect 시 detach. 캔버스 좌표 → 데스크톱 픽셀 스케일 변환 포함
- [x] 단위 +11 (scancode press/release/extended / mouse move / 좌·우·중간 버튼 press·release / wheel ± / horizontal wheel / wheel 클램프). 단위 52/52, 워크스페이스 3860/3860
- [x] 키보드/마우스 이벤트 캡처 (fast-path input 변환)
#### S5 — 클립보드 채널 (CLIPRDR) (Rust 측 완료)
- [x] `ClipboardChannel` — `justrdp-cliprdr::CliprdrClient` + `Arc<Mutex<ClipboardState>>` 공유 backend, `StaticChannelSet` 으로 wire 디코드/인코드 + chunk 라우팅
- [x] `ClipboardChannel::from_connection(&ConnectionResult)` — `cliprdr` 채널 자동 lookup, 미협상 시 `ChannelNotNegotiated` typed error
- [x] `process_channel_data(channel_id, payload) → Vec<Vec<u8>>` — `SessionEvent::Channel.data` 를 받아 CliprdrClient state machine 통과 후 응답 wire frames (TPKT-framed) 반환. 채널 id 미일치 시 빈 Vec 으로 무시
- [x] `set_local_format_data(format_id, bytes, format_name) → Vec<Vec<u8>>` — embedder 가 클립보드 데이터 push, 서버에 format-list announcement frames 반환. `CF_TEXT (1)` / `CF_UNICODETEXT (13)` / `CF_BITMAP (2)` 등 임의 format id
- [x] `take_remote_format_data(format_id) → Option<Vec<u8>>` — 서버가 보낸 클립보드 데이터 drain
- [x] `ClipboardState` — `local_format_data` / `remote_format_data` (`BTreeMap<u32, Vec<u8>>`) + `last_remote_formats`. UTF-16LE ↔ String 변환은 embedder (browser Clipboard API 가 native handling)
- [x] `justrdp-svc` 공개 API 확장 — `chunk_and_encode` + `pub mod chunk`
- [x] 단위 +4 (channel id lookup / 미협상 → typed error / 무관 channel id 무시 / set_local_format_data round-trip 통한 state). 단위 105/105, 워크스페이스 3913/3913
#### S5b — JsClient 클립보드 API + 데모 브라우저 클립보드 통합 (완료)
- [x] `JsClient` 가 connect 시 `cliprdr` 채널을 자동 attach (negotiated 안 된 경우 silent disable). `hasClipboard` getter 로 확인
- [x] `pollEvents` 가 `SessionEvent::Channel` 이벤트를 `ClipboardChannel.process_channel_data` 로 라우팅, 응답 wire frames 를 같은 transport 로 자동 송출
- [x] `setLocalClipboardText(string)` — UTF-16LE + NUL 인코드 후 `set_local_format_data(13, ...)` + format-list announcement frames 송출
- [x] `pollRemoteClipboardText() → string|null` — `take_remote_format_data(13)` 결과 UTF-16LE 디코드. trailing NUL 제거
- [x] 데모 페이지 `main.js` — `navigator.clipboard.readText/writeText` 통합. 캔버스 focus 시 browser → RDP 자동 푸시, 폴 루프가 RDP → browser 자동 푸시. Ctrl+C 후 50ms 지연으로 브라우저 클립보드 갱신 후 RDP 로 동기화
- [x] 빌드 그린 (단위 105/105 변동 없음, 워크스페이스 3913/3913, wasm32 그린)
#### S6 — 오디오 채널 (RDPSND PCM 16-bit) + Web Audio (완료)
- [x] `AudioChannel` (`audio.rs`) — `RdpsndClient` + `Arc<Mutex<AudioState>>` 공유 backend, `StaticChannelSet` 으로 wire 라우팅. `from_connection` 이 `rdpsnd` 채널 자동 lookup. `process_channel_data` 가 `SessionEvent::Channel.data` → 응답 wire frames 자동 생성
- [x] PCM-only 백엔드 — `WAVE_FORMAT_PCM` (16-bit, 1ch/2ch, 임의 sample rate) 만 accept. 다른 코덱 (ADPCM/Opus/AAC) 은 후속 (S6b) — browser-side 디코더 또는 server fallback 필요
- [x] `AudioFrame { format_no, sample_rate, channels, bits_per_sample, data: Vec<u8> }` + `take_frame()` FIFO
- [x] `JsClient.hasAudio` getter + `pollAudioFrame() → JsValue|null` (PCM16-LE bytes 가 `Uint8Array` 로 노출, 메타 필드 함께)
- [x] 데모 페이지 Web Audio 통합 — connect 시 `AudioContext` 생성, 폴 루프가 `pollAudioFrame()` 으로 frame drain → PCM16LE → Float32 변환 → `AudioBuffer.getChannelData` 채움 → `BufferSource.start` 로 연속 스케줄. disconnect 시 `audioCtx.close()`
- [x] 단위 +4 (channel id lookup / 미협상 typed error / 무관 channel id 무시 / FIFO take_frame 순서). 단위 109/109, 워크스페이스 3917/3917, wasm32 그린

#### S6b — ADPCM / DVI-ADPCM 디코드 (완료)
- [x] `SharedBackend` 에 per-format 디코더 캐시 (`BTreeMap<u16, Box<dyn AudioDecoder>>`) 추가 — `on_server_formats` 시점에 `justrdp-rdpsnd::make_decoder` 로 디코더 한 번만 인스턴스화, `format_no` (negotiated dense index) 로 키
- [x] `accept()` 확장 — `WaveFormatTag::PCM` (16-bit) 외에 `ADPCM` (0x0002, MS-ADPCM) 와 `DVI_ADPCM` (0x0011, IMA-ADPCM) 도 accept. 1/2 채널 + sample rate ≤ 48 kHz 가드
- [x] `on_wave_data` 가 디코더로 i16 샘플 추출 → little-endian PCM16 bytes 로 재포장 → `AudioFrame { bits_per_sample: 16, .. }` 큐잉. 디코더가 `Send` 트레잇 객체이므로 `RdpsndBackend: Send` 제약과 호환
- [x] 재사용 가능한 i16 디코드 버퍼 (`MAX_SAMPLE_RATE_HZ × MAX_CHANNELS × DECODE_BUFFER_SECS`) — wave PDU 당 새 alloc 없음
- [x] `make_decoder` 가 형식을 거부하면 (예: 잘못된 `extra_data`) 그 슬롯은 advertise 자체를 안 함 — 협상 단계에서 가짜 코덱 약속을 피함
- [x] 단위 +4 (PCM/MS-ADPCM/DVI 협상, PCM passthrough, MS-ADPCM 블록 디코드, 미협상 format_no 드롭). 단위 113/113, 워크스페이스 그린, wasm32 그린

#### S6c — RDPEAI 마이크 리다이렉션 (완료, JS 바인딩 후속)
- [x] `justrdp-rdpeai` 의존성 추가. `AudioInputClient` (이미 구현 + 테스트 완료) 를 `AudioChannel` 의 drdynvc 경로에 결선
- [x] `AudioInputProxy` (`DvcProcessor` wrapper) 추가 — 내부 `Arc<Mutex<AudioInputClient>>` 를 외부에서도 보유할 수 있도록 분기. drdynvc 디스패처가 채널별로 trait 호출을 직렬화하므로 lock 안전
- [x] `AudioChannel` 에 `audio_input: Option<Arc<Mutex<AudioInputClient>>>` 필드 + `drdynvc` 가 negotiated 된 경우 `AudioInputClient::new()` (PCM stereo/mono @ 44.1k/22.05k 16-bit advertise) 자동 등록
- [x] `audio_input_state() -> Option<AudioInputState>` — `recording` 플래그 + 협상된 PCM format (sample_rate / channels / bits_per_sample) + `frames_per_packet`. 서버가 Open PDU 를 아직 보내지 않았으면 `recording=false` + 첫 negotiated format 으로 best-effort 채움
- [x] `audio_input_pcm_frames(samples) -> Result<Vec<Vec<u8>>>` — captured PCM 을 1) `AudioInputClient::build_audio_messages` 으로 INCOMING_DATA + DATA DVC pair 생성 2) `DrdynvcClient::route_outbound` 으로 SvcMessage 래핑 (UDP tunnel 은 justrdp-web 에서 비활성이므로 SVC 경로만 정상, Tunnel variant 는 typed error) 3) `StaticChannelSet::encode_message` 로 청킹 + TPKT 프레이밍. 서버가 Open 안 한 경우 빈 Vec 반환 (silent drop, 폴링 친화)
- [x] `AudioChannelError` 에 `AudioInputUnavailable` (drdynvc 미협상) + `AudioInput(String)` (DVC 에러 wrapping) 변종 추가
- [x] 단위 +5 (state None when SVC-only / state Some when drdynvc / pcm_frames Unavailable error / pcm_frames silent drop pre-Open / DrdynvcClient 가 AUDIO_INPUT + AUDIO_PLAYBACK_DVC + AUDIO_PLAYBACK_LOSSY_DVC 셋 다 보유). 단위 129/129 (justrdp-web), 워크스페이스 그린, wasm32 그린
- [ ] **JS 바인딩 + 데모 mic capture** — 후속 S6c-js (`JsClient.audioInputState()` / `pushAudioInputPcm(bytes)`, `getUserMedia` + `AudioWorklet` 통합)

#### S6b2 — Opus / AAC injectable decoder (완료)
- [x] `pub trait ExternalAudioDecoder: Send` 추가 — `accepts(&AudioFormat) -> bool` + `decode(&AudioFormat, &[u8]) -> Vec<u8>` (반환값은 PCM16-LE bytes 약속). 빈 Vec 반환은 "drop silently" 시그널 (스트리밍 코덱 prefix buffering 용)
- [x] `SharedBackend` 가 `Option<Arc<Mutex<dyn ExternalAudioDecoder>>>` 보유 — 형식별 decoder cache 는 `enum DecoderSlot { Bundled, External(AudioFormat) }` 로 분기
- [x] `on_server_formats`: 1) bundled `make_decoder` 시도 2) 거부되면 external decoder 가 accept 시 advertise. 즉 Opus/AAC 가 협상되려면 embedder 가 decoder 를 주입해야 함. 채널 카운트 / sample rate 가드는 bundled / external 모두 동일하게 적용 (`shape_acceptable` 헬퍼 분리)
- [x] `on_wave_data`: `DecoderSlot::Bundled` 은 기존 PCM/ADPCM 경로, `DecoderSlot::External` 은 external decoder 호출 → 빈 출력은 silent drop, 그렇지 않으면 sample_rate / channels 를 format 에서 가져와 `AudioFrame` (always `bits_per_sample=16`) 큐잉
- [x] `AudioChannel::from_connection_with_external_decoder<D: ExternalAudioDecoder + 'static>` 새 컨스트럭터 — 기존 `from_connection` 은 그대로 (decoder = None). 주입된 decoder 는 `Arc<Mutex<dyn ...>>` 로 wrapping 되어 SVC + reliable DVC + lossy DVC 세 backend 모두에 공유됨 (stateful 코덱 일관성)
- [x] 단위 +5 (OPUS advertise / on_wave_data dispatch / 빈 출력 drop / PCM 시 external skip / from_connection_with_external_decoder 구조). 단위 124/124 (justrdp-web), 워크스페이스 그린, wasm32 그린
- [x] **JS 바인딩 / WebCodecs demo 통합** — 후속 S6b2-js (wasm 의 `js_sys::Function` 이 `Send` 가 아니므로 `forbid(unsafe_code)` 하 `Arc<Mutex<dyn Send>>` 로 직접 wrap 불가. wasm 단일 스레드 가정 하 별도 어댑터 모듈 필요)

#### S6d — DVC 모드 RDPSND (완료)
- [x] `AudioChannel::from_connection` 이 `rdpsnd` SVC 외에 `drdynvc` SVC 도 lookup. 둘 중 하나만 negotiated 되어도 통과 — embedder 가 `Config.channel("drdynvc", 0)` 만 advertise 하면 DVC-only 경로, 둘 다 advertise 하면 server 가 선택. 둘 다 없을 때만 `ChannelNotNegotiated`
- [x] `drdynvc` 가 negotiated 된 경우 `DrdynvcClient` 인스턴스에 `RdpsndDvcClient` (reliable, `AUDIO_PLAYBACK_DVC`) + `RdpsndLossyDvcClient` (lossy, `AUDIO_PLAYBACK_LOSSY_DVC`) 둘 다 register. 각 DVC 클라이언트는 자기 backend (`SharedBackend`) 를 갖되 같은 `Arc<Mutex<AudioState>>` 공유 → 어느 경로로 들어와도 같은 FIFO. lossy 는 UDP multitransport 가 없으면 서버가 선택하지 않으므로 사실상 reliable 만 쓰지만 advertise 는 둘 다
- [x] `process_channel_data` 가 `rdpsnd_channel_id` / `drdynvc_channel_id` 둘 다 매치 → `StaticChannelSet::process_incoming` 으로 라우팅. 무관한 id 는 빈 Vec 으로 silent ignore
- [x] `channel_id() -> u16` 제거 → `rdpsnd_channel_id() -> Option<u16>` + `drdynvc_channel_id() -> Option<u16>` 분리 (전송별 id 구분 필요)
- [x] `AudioChannelError::ChannelNotNegotiated` 메시지 갱신 ("neither rdpsnd nor drdynvc was negotiated")
- [x] 단위 +3 (drdynvc only / 둘 다 / 둘 다 없을 때 에러 / drdynvc id 가 process_channel_data 에 라우팅됨). 단위 119/119 (justrdp-web), 워크스페이스 그린, wasm32 그린

#### S7 — Native Embed Validation (Tauri / desktop binary 적용성 증명)

> **목적**: §11.3 의 transport-agnostic core (`WebClient<T: WebTransport>`,
> `BitmapRenderer`, `FrameSink`, `ClipboardChannel`, `AudioChannel`) 가
> wasm32-unknown-unknown 외에 **데스크톱 native 타깃 (x86_64-pc-windows-msvc /
> -unknown-linux-gnu / -apple-darwin)** 에서도 동일하게 동작함을 증명.
> 이 단계가 §11.5a Tauri Reference 의 prerequisite.

- [x] **`NativeTcpTransport`** (S7-1) — `tokio::net::TcpStream` 어댑터, `WebTransport`
      trait 구현 (send/recv/close async). wsproxy 없이 직접 RDP 서버 3389
      로 연결. `justrdp-web` crate 의 `feature = "native-tcp"` 게이트, tokio
      는 `[target.'cfg(not(target_arch = "wasm32"))']` 에 optional 로 선언되어
      wasm32 에서는 게이트가 no-op. `recv` 는 가용 바이트 그대로 반환 →
      driver 의 `recv_until_pdu` 가 TPKT/fast-path 재조립 (blocking client 와
      동일 경로). `from_stream` / `into_stream` 노출 (TLS 업그레이드 / 소켓 옵션
      제어용). EOF 시 sticky `closed` 플래그로 후속 호출 단락. 단위 +8 (loopback
      round-trip / chunked recv / EOF / post-close send / idempotent close /
      empty-send no-op / into_stream / set_recv_buf_size). `cargo test
      --features native-tcp` 137/137, default 129/129, wasm32 default + feature
      모두 그린
- [x] **`NativeTlsUpgrade`** (S7-2) — `tokio-rustls` 기반 async TLS 핸드셰이크.
      `feature = "native-tls"` (implies native-tcp). `dangerous_no_verify(server_name)`
      (RDP 기본 — mstsc 와 동일하게 self-signed 수용, NLA pubKeyAuth 가 별도로
      서버 신원 검증), `from_connector(connector, server_name)` (embedder 가
      직접 만든 rustls config 사용), `with_system_roots` 는 stub
      (TransportErrorKind::Other) — webpki-roots dep 추가는 follow-up. `NativeTlsTransport`
      (TlsStream<TcpStream> wrapper, NativeTcpTransport 와 동일한 recv/send 스타일).
      `impl TlsUpgrade<NativeTcpTransport>` 가 `into_stream()` → `connector.connect()`
      → `NativeTlsTransport` 로 변환 → `connect_with_upgrade` 통과. 단위 +6
      (DNS / IP server name accept, invalid server name reject, with_system_roots
      stub, supported schemes RDP-common cases, NoVerify shim placeholder). 143/143
      with `--features native-tls`, wasm32 + feature 도 no-op 통과
- [x] **NLA 어댑터 (native)** (S7-3) — `NativeCredsspDriver` 가 async
      `CredsspDriver<NativeTlsTransport>` 구현. blocking 의 `run_credssp_sequence`
      를 그대로 async 로 적응 (state machine 동일, I/O 만 async). 서버 SPKI 는
      `NativeTlsTransport::server_public_key()` 가 rustls 의 peer_certificates 에서
      추출 (`justrdp-tls::extract_spki_from_cert_der` 재사용, default-features=false
      로 rustls 중복 회피). NTLMv2 (사용자명/비밀번호) 는 `justrdp-connector`
      내장 NTLM 으로 직접 동작 — Kerberos 는 후속 (Windows SSPI / libkrb5).
      `feature = "native-nla"` (implies native-tls). Driver: SendNegoToken /
      WaitChallenge / WaitPubKeyAuth / SendCredentials / WaitEarlyUserAuth
      / Done 상태 모두 처리. `read_asn1_sequence` (DER SEQUENCE 재조립) +
      `handle_early_user_auth` (HYBRID_EX raw 4바이트 status vs fallback
      TsRequest 분기). 단위 +3 (driver 구성 / OS RNG 동작 / random 필드 채움).
      146/146 with `--features native-nla`, default 129/129, wasm32 + feature
      no-op 통과
- [x] **클립보드/오디오 native 백엔드 결선** (S7-4) — `AudioChannel` 와
      `ClipboardChannel` 모두 `from_connection_with_backend<B>(result, backend)`
      컨스트럭터 추가. 1) `ClipboardChannel`: `B: CliprdrBackend + 'static` 을
      `Box<dyn>` 으로 받아 `CliprdrClient::new` 에 직접 전달, 번들 `SharedBackend`
      / `ClipboardState` cache 우회 (state 자체는 보존되어 `set_local_format_data` /
      `take_remote_format_data` 호환). 2) `AudioChannel`: `B: RdpsndBackend + 'static`
      을 `Arc<Mutex<dyn RdpsndBackend>>` 로 wrap, 새 `BackendProxy` (per-call
      lock + delegate) 가 SVC + reliable DVC + lossy DVC 세 `RdpsndClient`
      모두에 주입되어 동일 backend 인스턴스 공유. embedder 가 backend 의
      Drop 한 번만 보면 됨. 새 `enum BackendMode { Bundled { external_decoder },
      External(handle) }` 으로 모든 컨스트럭터가 단일 build() 헬퍼 통과 → API
      확장 ergonomics 유지. 단위 +4 (clipboard custom backend / audio FIFO 우회 /
      proxy 4 callback delegate / SVC+DVC 모든 RdpsndClient 가 같은 backend 공유
      → Drop count = 1). default 133/133, `--features native-nla` 150/150,
      wasm32 default 빌드 그린
- [x] **단위 테스트 매트릭스** (S7-1/2/3) — `cargo test --features native-tcp`
      137/137, `--features native-tls` 143/143, `--features native-nla` 146/146.
      각 feature 가 wasm32 에서도 no-op 으로 빌드 통과 (target-aware cfg gates)
- [x] **통합 예제** (S7-5) — `crates/justrdp-web/examples/native_connect_demo.rs`
      (cargo `[[example]]` + `required-features = ["native-nla"]`). CLI 인자로
      `host:port`, username, password (옵션 domain) 받아 `NativeTcpTransport`
      → `NativeTlsUpgrade::dangerous_no_verify` → `NativeCredsspDriver` 체인을
      통과 → `ActiveSession::next_events` 5회 펌프 → `transport.close()`. 실서버
      (192.168.136.136) 매뉴얼 smoke 는 embedder 가 `cargo run --example
      native_connect_demo --features native-nla -- 192.168.136.136:3389
      <user> <pass>` 로 검증
- [x] **시각 렌더링 검증** (S7-5b) — `examples/native_render_demo.rs`. S7-5
      의 connect 체인 + `render_event` + 커스텀 `FrameSink` (`PngFrameSink`,
      Vec<u8> 백킹 + `peek_rgba` impl) 합성. 30 frame 펌프 동안 들어온 모든
      `SessionEvent::Graphics` 를 framebuffer 에 누적, 종료 시 `frame.png`
      (8-bit RGBA) 로 dump (`png 0.17` dev-dep, `image` 보다 가벼움). resize
      이벤트 시 framebuffer 재할당
- [x] **라이브 윈도우 통합** (S7-5c) — `examples/native_window_demo.rs`.
      winit 0.30 (`ApplicationHandler` 기반) + softbuffer 0.4 로 OS 윈도우 +
      소프트웨어 표면 업로드. 아키텍처: tokio thread (RDP 펌프) ↔ winit
      main thread (이벤트 루프) 사이를 `Arc<Mutex<Framebuffer>>` (ARGB
      packed, softbuffer 직접 소비) + `EventLoopProxy<AppEvent>` (Redraw /
      Disconnected user event) 로 연결. `WinitFrameSink` 가 incoming RGBA
      → 0xFF000000|RGB ARGB 변환 후 framebuffer 에 write, `flush()` 마다
      proxy 로 Redraw 시그널 전송 (배치당 1회만, 중복 spam 방지). winit
      `RedrawRequested` 핸들러는 `surface.buffer_mut()` 로 소프트버퍼 받아
      framebuffer copy 후 `present()`. CloseRequested → event_loop.exit().
      소프트버퍼 vs 프레임버퍼 사이즈 미스매치 시 top-left clipping (검정
      strip). 키보드/마우스 forwarding 은 의도적 out-of-scope (S7 범위는
      "1 frame 디코드"). 의존성: `winit 0.30`, `softbuffer 0.4` dev-dep.
      매뉴얼 smoke: `cargo run --release --example native_window_demo
      --features native-nla -- 192.168.136.136:3389 user pass`

#### S8 — Client Display-Control PDUs (`send_refresh_rect` / `send_suppress_output`)

> **rationale**: 실제 RDP 클라이언트는 윈도우 minimize → `SuppressOutput(allow=false)`,
> restore → `SuppressOutput(allow=true) + RefreshRect(전체 영역)` 시퀀스로 서버 측
> 트래픽을 제어한다. wasm/native/tauri 어떤 frontend 든 동일하게 필요한
> 라이브러리 API. PDU encode/decode (§3 L397-398) + 서버 수신 핸들러 (§11.1
> `ServerActiveStage` L2383) 는 이미 완비, 클라이언트 송신 헬퍼만 결손이라
> §11.3 S7 검증 중 visual proof 가 막힌 직접 원인. `send_input` /
> `shutdown` / `disconnect` 와 동일 위치(§11.3 ActiveSession)에 박는다.

- [x] `justrdp-session::x224_proc::encode_refresh_rect(user_chan, io_chan, share_id, areas)`
      — `RefreshRectPdu` → ShareDataHeader(0x21) → ShareControlHeader(Data) →
      MCS SendDataRequest. `encode_shutdown_request` 패턴 그대로
- [x] `encode_suppress_output(user_chan, io_chan, share_id, allow, rect)` —
      `SuppressOutputPdu` → ShareDataHeader(0x23). `allow=false` 시 rect 무시
- [x] `ActiveStage::encode_refresh_rect` / `encode_suppress_output` —
      connector state(share_id / user / io chan) 자동 캡처해서 위 두 함수 호출
- [x] `ActiveSession::send_refresh_rect(&[InclusiveRect])` /
      `send_suppress_output(allow, Option<InclusiveRect>)` — 기존
      `send_input` / `shutdown` 옆에 동거
- [x] 단위 테스트 — 양쪽 encode wire 바이트 검증 (server 쪽 이미 있는
      `wrap_client_share_data` 와 디코드 호환), `ActiveSession` 단위는
      MockTransport 로 send 호출 시 정확한 슬라이스가 transport.send 에 들어가는지
- [x] **시각 검증** — `examples/native_render_demo.rs` 에 connect 직후
      `session.send_refresh_rect(&[InclusiveRect{0,0,1023,767}])` 송신 추가,
      frame.png 에 실제 desktop 픽셀이 찍히는지 (이전 빈 검정 PNG 와 대조).
      **결과**: Windows Server 2019 잠금 화면 (산/하늘 배경 + 로딩
      스피너) 7 bitmap-update batch 가 PNG 에 그려짐 — 이전 빈 검정
      대비 100 KB 차이. 이 과정에서 fast-path bitmap decoder 의
      `updateType` prefix 누락 버그도 발견·수정 (별개 commit)

### 11.4 `justrdp-ffi` -- C/Python FFI Bindings  `[both]`
...
### 11.5 `justrdp-client` -- Reference Client Binary  `[M-Client]`

> **requires**: Phase 3~6 (전체 클라이언트 파이프라인), §5.6 async runtime,
> §11.3 S7 Native Embed Validation, §5.3.1 finalization fix
>
> **트랙 분리 동기**: §11.3 wasm 데모에서 이미 `BitmapRenderer` /
> `FrameSink` / `ClipboardChannel` / `AudioChannel` 이 89% 완성도로
> 동작한다. 이 자산을 native 에서 그대로 재사용하는 **Tauri-first 트랙
> (11.5a, P0)** 을 먼저 출시하고, 별도 winit/wgpu 자체 렌더러 트랙
> (11.5b, P2) 은 후속으로 분리한다. softbuffer/glutin 동시 지원은
> 의도적으로 잘라낸다 (wgpu 가 모든 데스크톱 타깃 커버).

#### 11.5a — Tauri Reference (P0, MVP 출시 트랙)

> **결과물**: Windows/Linux/macOS 데스크톱 RDP 클라이언트 한 바이너리.
> §11.3 S7 NativeTcpTransport 위에 Tauri webview 를 GUI 로 얹는다.
> 단일 모니터 + NLA/NTLMv2 + Standard codecs + 클립보드/오디오/RDPDR
> share 가 MVP scope.

- [ ] `crates/justrdp-tauri/` 신설 (workspace member, `cdylib` + `bin`)
- [ ] **bridge 레이어** — `tauri::command` 5종:
  - [ ] `connect(host, port, user, pass, domain) -> SessionHandle`
  - [ ] `send_input(handle, InputEvent)` (key/mouse/wheel)
  - [ ] `set_local_clipboard(handle, text)` / `poll_remote_clipboard(handle)`
  - [ ] `disconnect(handle)`
- [ ] **frame push 레이어** — `tauri::Window::emit("rdp:frame", ...)`
      로 RGBA blit 영역을 frontend 로 push. `BitmapRenderer` →
      `TauriFrameSink` (FrameSink trait 구현, `blit_rgba` 가 emit) 어댑터
- [ ] **session pump task** — §5.6 `AsyncRdpClient` + `tokio::spawn` 으로
      백그라운드 펌프, mpsc 로 GUI 측에 event 전달. `disconnect` 시
      cancel-safe shutdown
- [ ] **frontend (TypeScript + canvas)** — wasm 데모의 `main.js` 를
      Tauri event listener 형태로 포팅. `<canvas>` ImageData put_image_data,
      keydown/mousemove → `invoke("send_input", ...)`, 클립보드 동기화
- [ ] **클립보드/오디오 native 통합** — §11.3 S7 의 `BackendDriver` 주입
      포인트로 `justrdp-cliprdr-native` / `justrdp-rdpsnd-native` 결선
- [ ] **신뢰 모델** — `tauri.conf.json` allowlist 최소화 (network 만,
      fs/shell/dialog 차단). 서버 인증서 first-use trust prompt 는
      `tauri::dialog::ask` 로 노출
- [ ] **packaging** — Windows MSI + macOS .dmg + Linux AppImage 빌드
      파이프라인 (Tauri CLI default)
- [ ] **smoke test** — Windows Server 2019 (192.168.136.136) 에 NLA/NTLMv2
      로 연결, 5분 세션 유지, 클립보드 양방향, 사운드 PCM 재생, 키 입력,
      disconnect 클린업 확인

#### 11.5b — Native Renderer Reference (P2, 후속)

> **목적**: Tauri webview 의존 없이 GPU/소프트 렌더 직접 그리는 비교용
> 바이너리. 11.5a 가 안정화된 후 진행.

- [ ] CLI 인터페이스 (clap)
- [ ] `.rdp` 파일 지원
- [ ] 렌더링 백엔드: wgpu only (softbuffer/glutin 잘림)
- [ ] 윈도우 시스템 통합 (winit)
- [ ] 클립보드/파일/오디오 네이티브 통합 (11.5a 와 backend 공유)
- [ ] 멀티모니터 지원 (§9.1)
- [ ] 게이트웨이 연결 지원 (§10.1)
- [ ] 세션 녹화/재생 (디버깅용)

### 11.6 `justrdp-gateway` -- RD Gateway Server  `[M-Server]`
...
### 11.7 `justrdp-proxy` -- RDP Proxy  `[M-Server]`
...
- **Public release prerequisite (§11.5a Tauri 바이너리)**: Tier 1 + Tier 2
   가 무크래시여야 binary 배포 가능. fuzz 미충족 시 `--features
   experimental-release` 플래그 강제 + README 경고 노출
...
justrdp-core (foundation, no deps)
  ...
  │     │     │
  │     │     ├──▸ justrdp-async (§5.6.1 Phase 1 ✅ — WebTransport, WebClient<T>, ActiveSession<T>)
  │     │     │     │
  │     │     │     ├──▸ justrdp-tokio (depends on: blocking + tokio + async — v1 spawn_blocking wrap, native_* hosts)
  │     │     │     └──▸ justrdp-web (re-exports + wasm/render-only — audio, clipboard, render, input, canvas, js, websocket)
  │     │     │
  │     │     └──▸ justrdp-blocking (depends on: core, pdu, connector, tls — sync legacy)
  │     │
...
- [ ] **Finalization batched/out-of-order PDU drop fix** (§5.3.1) —
      Windows Server 2019 실서버 활성 세션 30s+ 무절단 유지. P0.
...
- [ ] **Native embed validation** (§11.3 S7): `WebClient<NativeTcpTransport>`
      가 데스크톱 3-OS 타깃에서 핸드셰이크 + 프레임 디코드 성공
- [ ] **§11.5a Tauri Reference Binary**: 단일 모니터 NLA/NTLMv2 5분
      세션 + 클립보드/오디오/입력/disconnect 클린업. Windows MSI /
      macOS .dmg / Linux AppImage 패키징 산출
- [ ] §11.5b Native renderer (winit + wgpu) 트랙: 동일 시나리오 재현
- [ ] **Async runtime** (§5.6) `justrdp-async` + `justrdp-tokio`:
      M1~M7 매트릭스 100% 재현, mock broker 시나리오 tokio 위에서 통과
...
> **Two views.** Below is the **technical phase view** (§4–§11 organised
> by protocol layer / crate). For the **product milestone view**
> (M-Client / M-Server scope and trigger conditions) see
> [§0 Product Milestones](#0-product-milestones) at the top of the file.

```
Phase 1 ▸ Foundation         justrdp-core, justrdp-pdu, justrdp-bulk
                             TPKT, X.224, MCS, GCC, PCB, Capabilities, Fast-Path, Drawing Orders
Phase 2 ▸ Connection         justrdp-connector, justrdp-tls, justrdp-blocking, justrdp-tokio
                             ...
                             Async runtime — §5.6 8-week unification (WebTransport family
                             promote → §5.6.1, feature parity → §5.6.2, gateway async
                             port → §5.6.3, AsyncRdpClient v2 → §5.6.4)  🚧
...
Phase 7 ▸ Transport          justrdp-rdpeudp, justrdp-rdpemt, justrdp-gateway (MS-TSGU)
                             ...
                             Note: §5.6.3 ports gateway client to async (M-Client path).
                             Server-side gateway is M-Server (§11.6).
Phase 8 ▸ Server+Ecosystem   justrdp-acceptor, justrdp-server                  [M-Server]
                             Server-side GFX encoding, multi-session, shadow   [M-Server]
                             justrdp-web (WASM)                                [M-Client]
                             justrdp-ffi (C/Python)                            [both]
                             justrdp-client (GUI), justrdp-tauri               [M-Client]
                             justrdp-gateway server, justrdp-proxy             [M-Server]
```
...
- [x] **S3b -- ActiveStage 송수신 wrap/unwrap**: `ServerActiveStage` 가
