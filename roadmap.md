...
#### S3d-6 — 추가 Primary Drawing Order 렌더 (부분 완료)
- [x] **PatBlt** with `BS_SOLID` brush + `PATCOPY` ROP — fore_color 채운 사각형 blit. Windows window-frame fill 의 가장 흔한 케이스 처리
- [x] 단위 +1 (PatBlt 라운드트립: PrimaryOrder::encode 로 wire format 만든 후 BitmapRenderer 통과 → fore_color RGBA 검증). 단위 69/69
...
- [ ] **Polyline / Polygon** — pen 색상 + 라인/폴리곤 라스터. 후속 — S3d-6e
