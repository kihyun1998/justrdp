...
- [ ] **`NativeTcpTransport`** — `tokio::net::TcpStream` 어댑터, `WebTransport`
      trait 구현 (send/recv/close async). wsproxy 없이 직접 RDP 서버 3389
      로 연결. `justrdp-web` crate 의 `feature = "native-tcp"` 게이트
- [ ] **`NativeTlsUpgrade`** — `justrdp-tls::RustlsUpgrader` 를 `TlsUpgrade`
      trait 으로 wrapping. `connect_with_upgrade` 가 native 에서도 핸드셰이크
      완주. 자체 서명 인증서 처리는 §5.4 `ServerCertVerifier` 재사용
