...
- [x] **GlyphIndex 텍스트 렌더** — `CacheGlyph V1` 파서 (orderType 0x03, cacheIndex/cx/cy/1bpp mask × cGlyphs entries) + glyph cache + GlyphIndex 22-field 본체 디코더 + cbData 명령 스트림 워커 (single index / 0xFE cache-id switch / 0xFF terminator). `composite_glyph_mask` 가 1bpp 마스크 → fore_color RGBA 합성. background rect / flAccel 수직·역방향 / CacheGlyph V2 는 후속 — S3d-6d3 (S3d-6d2)
