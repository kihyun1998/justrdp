...
- **멀티세션**: 런타임 관심사 (프로토콜 아님). `RdpServer` 는
  per-connection 이고, caller 가 tokio / `std::net` 으로 스폰.
  `examples/` 데모 추가는 **Phase 8 acceptance criteria**
  (이 파일 `- [ ] 멀티세션 동시 접속` 항목) 에서 추적됨.
- **Shadow / multi-party**: ✅ §9.13 Multiparty Virtual Channel
  (MS-RDPEMC) 에서 완료됨 -- encomsp SVC 위에서 다자 세션 /
  Shadow 세션 / View-only / Interactive 모드 지원.
