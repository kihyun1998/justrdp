...
- [ ] **MemBlt 기타 ROP** — `peek_rgba` 위에 ROP3 evaluator 작성 시 거의 자동 — S3d-6c4
- [ ] **Polygon (P/CB)** — Polyline 와 비슷하지만 scanline fill 필요 — S3d-6g2
...
- [ ] **DstBlt 나머지 ROP / DSTINVERT** — destination read-back 이 필요해 `FrameSink` 확장 (`read_rgba(rect)` 또는 별도 GPU surface buffer) 필요. 후속 — S3d-6h
...
- [ ] **DVC 모드 RDPSND** (`AUDIO_PLAYBACK_DVC` / `_LOSSY_DVC`) — 현재는 SVC 모드만. DVC 채널 통합 시 추가 — S6d
