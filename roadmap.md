...
- [ ] **PatBlt with `BS_PATTERN` / `BS_HATCHED`** — Secondary `CacheBrush` 디코드 필요 (현재 skip 됨), brush 8×8 pattern + ROP3 evaluation. 후속 — S3d-6b
...
- [ ] **GlyphIndex** — Secondary `CacheGlyph` 디코드 + glyph cache, 텍스트 렌더링 (1bpp glyph mask + foreground/background color). 후속 — S3d-6d
- [ ] **Polyline / Polygon** — `decode_polyline` / `decode_polygon` 헬퍼가 `justrdp-pdu` 에 미동봉. 디코더부터 만들고 라스터. 후속 — S3d-6g
