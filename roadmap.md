...
- [x] **PatBlt with `BS_PATTERN`** (8×8 inline 모노크롬) — `brush_hatch` (top row) + `brush_extra` (rows 1-7) 인라인 패턴 + brush_org 위상. PATCOPY 는 read-back 없이, PATPAINT/PATINVERT 등 RMW ROP 은 ROP3 evaluator + `peek_rgba` 통과. `BS_HATCHED` (predefined patterns) + `CacheBrush` 인덱스 패턴은 후속 — S3d-6b2 (S3d-6b 일부)
...
- [ ] **CacheBitmapV1 / V3** — 별도 wire layout, 후속 — S3d-6c5
...
- [-] **GlyphIndex** — body skeleton walker 완료 (cursor 진행), 텍스트 렌더 미구현 (`CacheGlyph` 파서 + 1bpp 마스크 + glyph fragment 명령 디코드 = 별도 substantial 작업) — S3d-6d2
- [x] **Polyline + Polygon (SC + CB skeleton)** — `decode_polyline` / `decode_polygon` 인라인 디코더 + Bresenham (Polyline) / scanline fill (Polygon SC, CB skeleton). justrdp-pdu 에 typed 디코더 upstream 은 별도 트랙 (S3d-6g3)
