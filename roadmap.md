...
#### S3c 이후 (예정)
- [-] WebSocket 전송 (브라우저 환경) — handshake (S1+S2) + 활성 세션 (S3a) + 렌더 (S3b) 통합 완료. 후속: stateful `JsClient` 핸들 (transport·session·sink 보존) 와 데모 페이지 (S3c)
- [ ] RLE 압축 / 15·16·24 bpp 색공간 / Orders / SurfaceCommands 디코드 — S3c
- [ ] `JsClient` (wasm-bindgen 핸들) — connect → poll loop → input → disconnect API 일체화 — S3c
