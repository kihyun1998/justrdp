...
```
    ...
    ┌─────────▼──────────────────────────────────────────────────────────┐
    │                       I/O Adapters                                │
    │  justrdp-blocking (sync)   justrdp-tokio (spawn_blocking wrap)   │
    │  justrdp-web   (WebTransport family — async on tokio + wasm)     │
    │  justrdp-tls (sync TLS upgrade)                                  │
    │  justrdp-async (planned: WebTransport promote at tokio v2)       │
    └────────────────────────────────────────────────────────────────────┘
              │
    ┌─────────▼──────────────────────────────────────────────────────────┐
...
| `justrdp-async`          | (§5.6.1, **redesigned 2026-04-29**) tokio v2 native port 착수 시 `justrdp-web::WebTransport` family 를 promote 하여 공용화. 원래 적혀 있던 `AsyncRead/Write/TlsUpgrader/CredsspDriver` shape 는 거부됨 — 자세한 사유는 §5.6.1 참조 |
| `justrdp-tokio`          | (§5.6.2 v1 **완료 2026-04-29**) `AsyncRdpClient` — `spawn_blocking` 으로 sync `RdpClient` wrap, mpsc/oneshot façade. Tauri/Iced/eframe 임베더용. v2 native port (tokio TcpStream + tokio-rustls) 는 별도 phase |
...
### 5.6 `justrdp-async` / `justrdp-tokio` -- Async I/O Runtime

> **requires**: 5.5 `justrdp-blocking` 안정화
> **검증**: 같은 실서버 핸드셰이크를 tokio 런타임 위에서 수행, M1~M7
>          매트릭스 100% 재현
> **동기**: §3 architecture diagram 과 §16 Dependency Policy 에는 등장하지만
>          현재 phase 항목이 없어 임베더(Tauri/Iced/axum 게이트웨이/DVC bridge)
>          가 직접 `spawn_blocking` 래퍼를 작성해야 함. 이 갭을 닫는다.

#### 5.6.1 `justrdp-async` (trait layer) — **rejected as designed (2026-04-29), redesigned around `WebTransport`**

> **상태 재검증 (2026-04-29):** 두 번째 async backend 는 실제로 이미
> 등장했음 — `crates/justrdp-web/` 가 wasm + native 양쪽 위에서 동작하는
> 두 번째 runtime 임. 그런데 `justrdp-web` 는 이 절에 적혀있던 trait set
> (`AsyncRead`/`AsyncWrite` + `AsyncTlsUpgrader` + `AsyncCredsspDriver`)
> 을 **의도적으로 거부하고 다른 추상화** (`WebTransport` —
> `recv() -> Vec<u8>` / `send(&[u8])` 의 message-oriented byte pipe) 를
> 만들어 사용 중. 이유: RDP-over-WebSocket 은 byte stream 이 아니라
> frame-oriented 이라 `AsyncRead`/`AsyncWrite` 는 모양 자체가 안 맞음.
> 따라서 아래 4 개 trait 항목은 잘못된 abstraction 으로 판정한다.

**원래 4 개 항목 — 거부 사유:**

- [~] ~~`AsyncRead` / `AsyncWrite` trait~~ — **rejected**: `tokio::io` /
  `futures-io` 가 사실상 표준이고, `justrdp-web::WebTransport` 는
  메시지 기반이라 두 standard 어디에도 안 맞음. 우리가 또 정의해도
  표준 alias 또는 사용처 없는 third flavor 가 됨.
- [~] ~~`AsyncTlsUpgrader` trait~~ — **rejected**: `tokio-rustls`
  /`async-rustls` 가 이미 같은 역할을 하고, `justrdp-web` 은 wasm 환경
  에서 TLS 를 브라우저(`wss://`) 에 위임하기 때문에 양쪽 backend 가
  공통적으로 구현할 만한 surface 자체가 존재하지 않음.
- [~] ~~`AsyncCredsspDriver` trait~~ — **rejected**: JustRDP 는 SSPI /
  libkrb5 를 사용하지 않음. `CredsspSequence::step()` 이 pure no_std
  Rust (`crates/justrdp-connector/src/credssp/`) 라 "async CredSSP" 는
  단순한 IO wrapping 이고, `blocking/credssp.rs:33-131` 와
  `web/native_nla.rs:66-161` 비교 시 trait 으로 합쳐서 얻는 것이 거의
  없음 (state machine 자체가 connector 안에서 끝나기 때문).

**올바른 abstraction (`§5.6.2 v2` 착수 시점에 promote):**

- [ ] `WebTransport` family 를 `justrdp-web` 에서 공용 crate (잠정명
  `justrdp-transport` 또는 본 절의 `justrdp-async`) 로 promote.
  - `WebTransport` (`recv()`, `send(&[u8])`, `close()`) — message-oriented
    byte pipe (`crates/justrdp-web/src/transport.rs:31-53`)
  - `TlsUpgrade<T: WebTransport>` — `WebTransport → WebTransport`
    transformer. tokio 측은 `tokio-rustls` 위에 얇게 wrap, web 측은
    `with_external_tls(true)` no-op (브라우저 위임)
    (`crates/justrdp-web/src/driver.rs:537-549`)
  - `CredsspDriver<T: WebTransport>` — `&mut Connector` + `&mut T` 받아
    NLA 단계 진행 (`crates/justrdp-web/src/driver.rs:575-585`)
- [ ] §5.6.2 v2 native tokio port 가 이 trait set 의 첫 비-web 구현체.
  같은 `WebClient<T>`/`ActiveSession` pump 를 공유 → ~5000 LoC 중복 제거.
- [ ] 착수 trigger: §5.6.2 v2 native port 가 RFC/실제 작업 단계 진입.
  그 전엔 `justrdp-web` 안에 그대로 두는 것이 정확함 (pre-promote 시
  소비자가 1 개라 abstraction 모양이 굳어지기 전에 강제 됨).
...
- [ ] §13.4 (신설) — Tauri/Iced/eframe 같은 native UI runtime 에서
      `AsyncRdpClient` 를 mpsc/EventBus 로 묶어 frame/event 를 webview/
      surface 로 push 하는 표준 패턴 코드 샘플. 단일 모니터 + 단일 윈도우
      기준 minimal viable embed.
