...
#### S3d-5 — NSCodec + ClearCodec + AVC444 코덱 통합 (부분 완료)
...
#### S3d-5b — AVC420 / AVC444 디스패치 (embedder-injected decoder, AVC420 완료) (부분 완료)
...
- [x] `set_avc420_codec_id(u8)` / `set_avc444_codec_id(u8)` — 서버 capability negotiation 결과 codec_id 등록
- [x] **AVC420** dispatch — Surface Bits `bitmap_data` 를 H.264 Annex B 단일 액세스 유닛으로 디코더에 투입, `Yuv420Frame` → `yuv420_to_bgra` (BT.709 full-range) → BGRA → RGBA → blit. 16-aligned 코딩 사이즈 vs 디스플레이 사이즈 차이 처리
- [x] `RenderError::Avc(AvcError)` / `AvcDecoderMissing` / `AvcFrameUnavailable` / `Avc444NotImplemented` typed errors
- [x] 단위 +5 (AVC420 round-trip with FakeAvcDecoder / 디코더 미주입 시 typed missing / 디코더가 None 반환 시 frame-unavailable / AVC444 typed not-impl / clear_avc_decoder disarm). 단위 65/65, 워크스페이스 3873/3873
- [ ] **AVC444 metablock 파싱** — MS-RDPEGFX 2.2.4.4 region map + dual-substream main/aux 디코드 (luma + chroma aux 합성 → `combine_avc444_planes` / `v2`). 현재는 typed not-implemented 으로 surface — S3d-5c
