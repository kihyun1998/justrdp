...
```
    ...
    │  justrdp-tokio v1 ✅ (spawn_blocking wrap); justrdp-web ✅ (own    │
    │  WebTransport pump, not yet promoted); justrdp-async 🚧 Phase 1.   │
    └────────────────────────────────────────────────────────────────────┘
              │
    ┌─────────▼──────────────────────────────────────────────────────────┐
...
| `justrdp-async`          | (§5.6.1, planned — Phase 1 of §5.6 unification) `WebTransport` family + `WebClient<T>` 펌프를 `justrdp-web` 에서 promote 한 공용 trait/impl crate. `no_std + alloc`. tokio/wasm/sync 모든 backend 의 공통 layer. `AsyncRead/Write/TlsUpgrader/CredsspDriver` 형태는 2026-04-29 거부됨 — §5.6.1 참조 |
| `justrdp-tokio`          | (§5.6.2 v1 **완료 2026-04-29**, v2 §5.6.4 planned) v1: `AsyncRdpClient` `spawn_blocking` wrap. v2: `WebClient<NativeTcpTransport>` 직결 — Send+Sync, 게이트웨이/재연결/리다이렉트 포함, fan-out 친화. Tauri/Iced/eframe 임베더용 |
...
| `justrdp-web`     | WASM 바인딩 (브라우저 RDP 클라이언트, §11.3 S7 native embed 포함) |
...
#### 5.6.1 Phase 1 — `WebTransport` family promote (1주, ~2700 LoC 이동)

- [ ] `crates/justrdp-async/` 신설 (`no_std + alloc`, 워크스페이스 등록)
- [ ] `WebTransport` (`recv()` / `send(&[u8])` / `close()`) trait — message-oriented byte pipe. `justrdp-web/src/transport.rs:31-53` 에서 이동.
- [ ] `TlsUpgrade<T: WebTransport>` trait — `WebTransport → WebTransport` transformer. `justrdp-web/src/driver.rs:537-549` 에서 이동.
- [ ] `CredsspDriver<T: WebTransport>` trait — `&mut Connector` + `&mut T` 받아 NLA 단계 진행. `justrdp-web/src/driver.rs:575-585` 에서 이동.
- [ ] `WebClient<T>` 펌프 (~1222 LoC) 와 `ActiveSession<T>` (~557 LoC) 이동.
- [ ] `NativeTcpTransport` / `NativeTlsUpgrade` / `NativeCredsspDriver` 를 `justrdp-web` → `justrdp-tokio` 로 이동 (~960 LoC). `justrdp-web` 은 wasm 전용 코드 (`canvas.rs`, `js.rs`, `websocket.rs`, `audio.rs`, `clipboard.rs`) 만 남음.
- [ ] 모든 기존 테스트 통과 검증 — `cargo test --workspace` + `wasm-pack test --headless --firefox` 양쪽.

**검증 기준**: 코드 라인 net 이동만, 동작 변화 0. `justrdp-web` examples 와 `native_connect_demo.rs` 가 동일하게 동작.
...
justrdp-core (foundation, no deps)
  ...
  │     │     │
  │     │     ├──▸ justrdp-blocking (depends on: core, pdu, connector, tls)
  │     │     │     └──▸ justrdp-tokio (depends on: blocking + tokio — v1 spawn_blocking wrap)
  │     │     │
  │     │     └──▸ justrdp-web (depends on: core, pdu, connector — WebTransport family)
  │     │           │
  │     │           └──▸ justrdp-async (planned §5.6.1 — promote WebTransport at tokio v2)
  │     │
