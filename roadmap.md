...
#### S3b 이후 (예정)
- [-] WebSocket 전송 (브라우저 환경) — handshake (S1+S2) 와 활성 세션 펌프 (S3a) 둘 다 연결 완료. 후속: 비트맵 디코드 → `FrameSink` 통합 (S3b)
- [ ] Canvas/WebGL 렌더링 (`FrameSink` trait + Canvas 2D 레퍼런스, 비트맵 fast-path 디코드, JsClient 핸들 신설) — S3b
