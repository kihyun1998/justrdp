...
- [x] **NLA 어댑터 (native)** (S7-3) — `NativeCredsspDriver` 가 async
      ...
      no-op 통과
- [ ] **클립보드/오디오 native 백엔드 결선** — `justrdp-cliprdr-native` /
      `justrdp-rdpsnd-native` 를 `ClipboardChannel` / `AudioChannel` 의
      `BackendDriver` trait 뒤에 주입. browser Clipboard/WebAudio 와 동일
      ergonomics. **현재 상태**: native embedder 가 직접 `RdpsndClient::new(custom_backend)`
      를 만들어 `StaticChannelSet` 에 넣으면 `AudioChannel` 우회로 native
      backend 를 쓸 수 있음 (S6 의 `RdpsndBackend` trait 가 이미 그 길을
      열어 둠). 정식 `BackendDriver` 추상화는 follow-up — S7-4
