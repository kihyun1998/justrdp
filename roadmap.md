...
| `justrdp-async`          | (§5.6, 미구현) async I/O trait 추상화: `AsyncRead`/`AsyncWrite`/`AsyncTlsUpgrader`/`AsyncCredsspDriver` — blocking API가 안정된 후 미러   |
| `justrdp-tokio`          | (§5.6, 미구현) tokio 백엔드: `AsyncRdpClient`, `tokio::net::TcpStream`, `tokio-rustls`. Tauri/Iced/eframe 임베더용         |
