...
- [ ] **클립보드/오디오 native 백엔드 결선** — `justrdp-cliprdr-native` /
      ...
      ergonomics
- [ ] **단위 테스트 매트릭스** — `cargo test --features native-tcp` 기준
      (a) MockTransport 9건 + (b) NativeTcpTransport 1건 (loopback 핸드셰이크,
      `#[ignore]` 옵션)
- [ ] **통합 예제** — `examples/native_embed_demo` (winit 1 윈도우 +
      `softbuffer` 1 surface). 실서버 (192.168.136.136) 핸드셰이크 + 1
      frame 디코드 + 1 키 입력 + disconnect 까지. 출력은 PNG dump (시각 검증)
