...
- [ ] **추가 Primary Drawing Order 렌더** — PatBlt brush / MemBlt bitmap cache / GlyphIndex / Polyline / Polygon 등. mstsc legacy 트래픽에서 textured fill / 글리프 렌더링 — S3d-6
- [ ] **NLA / SSL upgrade** (`EnhancedSecurityUpgrade` 상태 처리) — `justrdp-tls` 를 `WebTransport` 위에 얹는 어댑터 + `WebClient` 가 SSL 선택 시 자동 TLS 핸드셰이크 + post-TLS 스트림으로 swap. CredSSP/NLA 는 그 다음 — S3-NLA (별도 트랙, 우선순위 미정. mstsc 와 NLA 강제 서버 연결에 필수)
