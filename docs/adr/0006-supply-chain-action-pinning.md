...
The per-PR build/test/clippy workflow was dropped (commit `2ca6c87`, "No longer running CI in this repo") out of a concern that SHA-pinning its actions would be unmanageable to keep current by hand. Issue #99 brings CI **back** — the **fuzz** lane (ADR-0008 §2), a **supply-chain** check, and the restored **test** gate — and that pinning concern is exactly what this ADR resolves (§3: `fix` + Dependabot), so it no longer blocks any of them. Reintroducing workflows reopens a supply-chain question that the dangling "ADR-0006" references in issues #99 and #101 never actually wrote down.
...
- **No CI at all (keep the post-`2ca6c87` state).** Rejected: libFuzzer cannot run on the maintainer's Windows host (ADR-0008 §Context), so "no CI" means "no fuzzing"; and the concern that retired the old gate — keeping pinned actions current by hand — is removed by `just-shield fix` plus the github-actions Dependabot ecosystem, so the original objection no longer holds.
