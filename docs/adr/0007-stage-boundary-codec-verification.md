...
- **Synthetic-only verification is an explicit ceiling for codecs the VM never emits.** Where no real-server bitstream can be captured (CAVIDEO today), the acceptance bar is the synthetic oracle and ~~that limit is stated in the issue, not silently skipped~~ **— measured false while working #225: #58 is the CAVIDEO issue and states no such limit. The ceiling is stated in `justrdp-codecs/tests/differential_rfx.rs:14-16` instead (*"the real VM never emits CAVIDEO ... so synthetic streams are the corpus"*), which is a better home than an issue because it sits where the reader is. This clause is retained struck through rather than deleted: it is the second measured instance of a record asserting an obligation lives somewhere it does not — [ADR-0011](0011-zero-ironrdp-terminal-state.md)'s 2026-08-26 amendment is the first, and the pair is recorded in [`docs/map/territory/verification-harness.md`](../map/territory/verification-harness.md) §Known holes.** If such a server is later found, a real-VM smoke test is added then.
