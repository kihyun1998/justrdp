...
- Amended: 2026-08-21 — the 2026-06-19 amendment's *"all"* was a **module** claim reported as an entry-point one, and #230 measured the gap. See the corrected Consequences bullet below.
- Date: 2026-06-18
...
- **Implemented across all decoders (issue #98, landed).** The RLE decoder's `decompress_never_panics_on_arbitrary_input` was the tracer bullet; the no-panic property now also covers `planar`, `clearcodec`, `rfx` and `pointer` in `justrdp-codecs`, and the server-controlled `justrdp-pdu` parsers (`egfx`, `fastpath`, `capability`, `license`, `mcs`). The three codec properties whose class FreeRDP has a real OOB CVE for are grounded in it (`rle` → CVE-2024-32460, `planar` → CVE-2024-32458, `clearcodec` → CVE-2020-11040).
- **Corrected 2026-08-21 (#230): that list is by *module*, and the sentence it replaced — "the `fuzz/` lane carries libFuzzer targets for the same entry points" — was false.** A module appearing in both rosters says a file of that name exists on each side, not that the same `pub fn`s are driven. Measured while closing #230: `license` named `ServerLicenseRequest::decode` alone, so `LicensePreamble` / `LicenseError` / `PlatformChallenge` / `NewLicense` had neither artifact while the module read as covered; `pointer` named `justrdp_codecs::pointer::decode_pointer`, whose header fields arrive *already parsed*, so the `justrdp-pdu` parser that produces them was driven by nothing. Both are closed. **The obligation this ADR states is therefore per entry point, not per module**, and the derivation that enumerates it — with the two commands and the gap between them — lives in [untrusted decode never panics](../map/invariant/untrusted-decode-never-panics.md) rather than in a list here, because a list here is what went stale.
