...
- [ ] **M** — Surface Commands: SetSurfaceBits / StreamSurfaceBits / FrameMarker; frame-ack. **A `destRect` is the bitmap's dimensions, not just its position** — `[MS-RDPEGFX]` 2.2.2.1 makes it *"the dimensions (width and height) of the bitmap data encapsulated in the bitmapData field"* and 2.2.1.2 bounds its fields at `u16` with no maximum, so on every arm whose codec expands its input the rectangle alone sizes the allocation and the server picks it; bound it above the codec dispatch, not per-arm (#263, §11f). **Two codec-id enums (don't conflate):** `WireToSurface1` carries **Codec1Type** (0x0 Uncompressed, 0x3 RemoteFX, 0x8 ClearCodec, AVC420/AVC444); `WireToSurface2` carries **Codec2Type** (0x9 RemoteFX Progressive). *ironrdp ref: session/fast_path.rs (legacy surface-command dispatch is partial — ClearCodec/Progressive not wired at the session level; EGFX path handles them via `ironrdp-egfx` + graphics codecs).*
...
- [x] **M(β)** — **RemoteFX Progressive decode**: TileSimple/First/Upgrade, DAS sign state, SRL, progressive quant, multi-pass coefficient accumulation, final DWT+colour. **ironrdp is NEAR decode-complete** (corrected, source-verified): `graphics/progressive.rs` ships both the low-level passes (`decode_first_pass`/`decode_upgrade_pass`) AND a high-level `TileState` (cross-pass store) + `decode_first`/`decode_upgrade` + `reconstruct_to_rgba` (full tile→RGBA) + a `SurfaceTiles` grid. *Our work = wire `WireToSurface2` payload → `SurfaceTiles`/`TileState` → `reconstruct_to_rgba` → `FrameUpdate`, plus surface bookkeeping.* ← the β work; this server uses it; smaller than first thought. **Superseded 2026-08-10 (epic #158):** the "our work is only the wiring" framing was the bootstrap-era plan and is no longer the decision — ADR-0003's rewrite order self-owns Progressive, so `ironrdp-graphics::progressive` becomes the differential *oracle*, not the implementation. Slice 1 (#167) landed the self-owned block-stream parser (`justrdp_pdu::rfx::progressive`); slices #168–#171 owned the decode passes, the store's lifecycle and the full-pipeline assembly, and **#172 closed the epic** by wiring that decoder into `justrdp::egfx` and retiring the bootstrap delegation — so the checkbox is done and `ironrdp-graphics` stays at runtime for zgfx alone (#189).
- **(§0 trap, #271)** **Advertising a higher EGFX capability version can black the screen with
  no error anywhere.** Measured live 2026-09-02. `[MS-RDPEGFX]` 1.5.1 makes `CAPVERSION_105`,
  `CAPVERSION_106` and `CAPVERSION_107`-without-`SCALEDMAP_DISABLE` a **MUST** to process
  `RDPGFX_MAP_SURFACE_TO_SCALED_OUTPUT_PDU` (`cmdId` 0x0017). Offered a ladder through 10.6 the
  test VM confirmed **10.6** and mapped the surface with that command; `GraphicsProcessor`
  skips it as unknown, so the surface was never mapped to an output and **0 frames** reached
  the host — while the session stayed up, the channel stayed open and `StartFrame`/`EndFrame`
  kept bracketing. Every assertion on the caps handshake passes in that state. Stopping the
  ladder at **10.4** confirms 10.4 and paints (110 frames, 5 457 298 px over a 1 280x800
  desktop). Two further numbers from the same sitting, neither obvious: this server's scaled
  map is **1:1** (`target` equals the surface's own dimensions, so honouring the command needs
  no resampler — `ironrdp-egfx` discharges the same MUST by bookkeeping), and it **honours a
...
- [x] **O** — zgfx (RDP8 bulk compression for EGFX payloads): 2.5MB LZ77 history, segmented. History persists across frames. **Self-owned in #189** (`justrdp-codecs::zgfx`), which deleted the `egfx-bootstrap` feature and with it the last `ironrdp-graphics` runtime dependency. The "decode-complete in `graphics/zgfx`" note was the bootstrap-era framing and is superseded: that decompressor is now the differential oracle, and it is the one whose panics on crafted input (5 of 7 probes, reaching the core) made the swap a robustness fix as much as an ownership one.
...
- [ ] **M — Virtual channel abstraction seam (ADR-0005).** *(Superseded for static channels by #307: the host receives `SessionOutput::ChannelData` and sends through `send_channel`, and no `SvcProcessor` is registered. See `docs/map/territory/virtual-channels.md`.)* Channels are `ironrdp`'s `SvcProcessor`/`DvcProcessor` instances registered on the connector. Remote→host data (frame updates, clipboard content, etc.) flows through closures captured at construction; host→remote commands arrive on `mpsc` into the session loop. No custom `ChannelHandler` trait — reuse what ironrdp provides, per ADR-0005.
...
- **§11c is cited for a rule it does not contain, and the citation is load-bearing**
  (noted 2026-08-31, #263). `justrdp::session`'s `apply_bitmap` cites *plan.md §11c* for an
  **allocation** bound ("a tiny malicious PDU declaring 65535x65535 would otherwise force a
  multi-gigabyte allocation"), and §11c contains no allocation rule at all — the closest it
  has is the sentence below, *decode failures (malformed bytes) = fatal*, which settles the
  **outcome** and nothing else. The **trigger** belongs to §11f, whose bullet prescribed the
  opposite outcome (*warn and skip*) until it was corrected in the same sweep. So the code
  follows §11c's outcome under §11f's trigger, and neither section said so. Read them
  together for this class; #263's EGFX `destRect` bound is the second site to land on it.
...
- [ ] **M — Input validation / untrusted bounds.** All server-supplied values that control memory allocation or offset calculation must be bounds-checked: bitmap dimensions (max screen size), palette entries (palette size), codec output size, frame counts, etc. A malicious server sending `width=65535, height=65535, bpp=32` would allocate 16 GB. **Policy:** bounds checks in every decoder; log a warn and skip the frame if bounds exceeded. *ironrdp ref: `DecodedImage::new()` validates size; decoder primitives check bounds.*
  ...
  **Measured 2026-08-31 (#263), and two clauses of the above are now wrong rather than
  hypothetical.** *(1) The 16 GB is real and it is not the 32-bit case.* A **93-byte** EGFX
  WireToSurface1 payload declaring `destRect = (0,0,65535,65535)` returned `Ok` on
  `x86_64-pc-windows-msvc` after allocating 17_179_344_900 bytes in 18.9 s, and panicked on
  `i686-pc-windows-msvc`. The 64-bit row is host-conditional in the direction that makes it
  worse: where commit is short, `alloc_zeroed` fails, which is `handle_alloc_error` and
  therefore **`abort`** — uncatchable, and fatal to the host process rather than to the RDP
  task. *(2) "log a warn and skip the frame" is not what landed, deliberately.* Both sites
  that now carry this bound return a **typed error that is fatal for the path**:
  `justrdp::session`'s `apply_bitmap` (`TS_BITMAP_DATA` exceeding the negotiated desktop) and
  `justrdp::egfx`'s WireToSurface1 arm (a `destRect` larger than any admissible surface).
  Warn-and-skip is the tolerance for a *decoder limitation*, and an allocation bound is not
  one — the split is written at the `decode_wts1` dispatch and in §11c below.
  *(3) "max screen size" is not the available bound everywhere.* A `destRect` is in **surface**
  coordinates, not desktop coordinates, so the desktop size cannot bound it; the number used
  is `MAX_TOTAL_SURFACE_BYTES` (256 MiB), derived from the fact that `CREATE_SURFACE` already
  refuses any surface set exceeding it. See
...
- [x] **M — SetKeyboardIndicators PDU (TS_SET_KEYBOARD_INDICATORS_PDU).** Done in #305, session leg only. `[MS-RDPBCGR]` 2.2.8.2.1.1: 2-byte `unitId` (the client SHOULD ignore it, so it is not decoded) + 2-byte `ledFlags`, whose bits are the Synchronize event's: SCROLL(0x0001), NUM(0x0002), CAPS(0x0004), KANA(0x0008). *This line used to say a 4-byte unitId and CAPS=0x1 / SCROLL=0x4 — both wrong against the spec.* Surfaced as `SessionOutput::KeyboardIndicators`; what the host does with it (LEDs, input filtering) is policy. See `docs/map/territory/input-scancodes.md`.
- [ ] **O — SetKeyboardImeStatus PDU (TS_SET_KEYBOARD_IME_STATUS_PDU).** `[MS-RDPBCGR]` 2.2.8.2.2.1: 2-byte `UnitId` (the client SHOULD ignore it) + 4-byte `ImeState` (`IME_STATE_CLOSED` 0 / `IME_STATE_OPEN` 1) + 4-byte `ImeConvMode` (`IME_CMODE_*` flags: NATIVE 0x1, KATAKANA 0x2, FULLSHAPE 0x8, ROMAN 0x10, CHARCODE 0x20, …) — 10 bytes, matching FreeRDP `rdp_recv_server_set_keyboard_ime_status_pdu`. *This line used to say `2-byte unitId + 2-byte imeId + 4-byte imeState` (8 bytes); there is no `imeId` (#305).* IME (Input Method Editor) for CJK/complex scripts. *ironrdp ref: ShareDataPdu::SetKeyboardImeStatus(Vec<u8>).* **Niche:** only relevant if supporting CJK; track & pass to IME subsystem.
- [ ] **M — Activation-time sync.** On session activation (Font Map received), client sends Synchronize input event (ScanCode(0x00)) with toggle flags matching local keyboard state, **before** user presses keys. Server responds with SetKeyboardIndicators if different. *ironrdp ref: input/sync.rs, finalization_messages.rs.* *Measured in #305: the WS2022 test VM sent no SetKeyboardIndicators to justrdp for any Synchronize or lock-key stimulus, but did send one to FreeRDP — so what provokes it is not established.*
...
- [x] **O — zgfx bulk-compression vectors.** Done in #189, and the shape landed differently from what this line imagined: the primary vector is the **`[MS-RDPEGFX]` sample**, which FreeRDP and `ironrdp-graphics` reproduce byte-identically (so it is an expectation derived from neither), and the breadth comes from a sequence differential in `justrdp-codecs/tests/differential_zgfx.rs`. Capturing real payloads was not needed: the VM's traffic was measured instead (25 messages, all single + compressed, longest match distance 133 937), which is what bounds the claim rather than what proves it.
...
- [x] **O — Fuzz zgfx decompression.** Done in #189: `fuzz/fuzz_targets/zgfx.rs` plus three no-panic proptests, one undirected and two prefixed with a valid wrapper (a descriptor byte and a compression-type nibble gate the token decoder behind 1-in-2048 of random prefixes). **The gotcha on this line was right and is now a constant**: `MAX_SEGMENT_OUTPUT` caps what one compressed segment may expand to and `MAX_DECOMPRESSED_BYTES` caps a multipart's declared size — neither reference implementation has either bound, and a ~20-byte segment can otherwise demand gigabytes.
