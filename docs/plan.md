...
- [ ] **M — Activation-time sync.** On session activation (Font Map received), client sends Synchronize input event (ScanCode(0x00)) with toggle flags matching local keyboard state, **before** user presses keys. Server responds with SetKeyboardIndicators if different. *ironrdp ref: input/sync.rs, finalization_messages.rs.* *Measured in #305: the WS2022 test VM sends no SetKeyboardIndicators for any Synchronize or lock-key stimulus, so "if different" is not this server's behaviour.*
